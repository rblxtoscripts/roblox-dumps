...
local t = {
	Enchants = {
		...
		Champion = {
			Name = "Champion",
			Rarity = "Nightmare",
			String = "+250% damage, +80% range, and +20% slower cooldown",
			Multi = {
				Damage = 3.5,
				Range = 1.8,
				Speed = 1.2
...
function t.Enchant(p1, p2, p3) --[[ Enchant | Line: 87 | Upvalues: v1 (copy), t2 (copy), ReplicatedStorage (copy), t (copy), Quests (copy) ]]
	...
	local v2 = if v12.Data.Gamepasses.VIP then 0.17 else 0.5
	if t2[p1.Name] then
		return
	end
	t2[p1.Name] = true
	...
	if t.Chances[v4] <= 0.1 then
		Quests.DoTask(p1, "RollLowEnchant", 1)
	end
	ReplicatedStorage.Remotes.Misc.Enchant:FireClient(p1, v4, p2)
end
function t.CheckShiny(p1, p2, p3) --[[ CheckShiny | Line: 160 | Upvalues: LocalPlayer (copy), v1 (copy) ]]
	if p1 then
...
function t.GetStatMulti(p1, p2, p3, p4) --[[ GetStatMulti | Line: 187 | Upvalues: LocalPlayer (copy), v1 (copy), t (copy) ]]
	local v12 = if LocalPlayer then v1.GetData() else v1.Profiles[p1]
	if not v12 then
		return
	end
...
if not LocalPlayer then
	ReplicatedStorage.Remotes.Misc.Enchant.OnServerEvent:Connect(function(p1, p2, p3) --[[ Line: 216 | Upvalues: t (copy) ]]
