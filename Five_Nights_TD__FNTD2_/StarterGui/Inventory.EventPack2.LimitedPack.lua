...
function update() --[[ update | Line: 12 | Upvalues: ClientState (copy), v1 (copy), Formatter (copy) ]]
	...
	v1.BuyCoins.Info.Text = "$" .. Formatter.FormatCompact((if v12.BoughtNewSummerPack2 then v12.BoughtNewSummerPack2 else 0) * 100000 + 200000) .. " Beach Balls"
end
v1.Buy.MouseButton1Click:Connect(function() --[[ Line: 23 | Upvalues: MarketplaceService (copy), LocalPlayer (copy), Robux (copy) ]]
	MarketplaceService:PromptProductPurchase(LocalPlayer, Robux.Products.EventPack2)
end)
pcall(function() --[[ Line: 26 | Upvalues: v1 (copy), MarketplaceService (copy), Robux (copy) ]]
	v1.Buy.Info.Text = ("R$ %*"):format(MarketplaceService:GetProductInfo(Robux.Products.EventPack2, Enum.InfoType.Product).PriceInRobux)
end)
v1.BuyCoins.MouseButton1Click:Connect(function() --[[ Line: 31 ]]
	game.ReplicatedStorage.Remotes.Misc.EventPassTouch.Pack2:FireServer()
end)
game.ReplicatedStorage.Remotes.Misc.EventPassTouch.Pack2.OnClientEvent:Connect(function() --[[ Line: 35 | Upvalues: v1 (copy), Tween (copy) ]]
