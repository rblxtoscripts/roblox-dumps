...
function t.HasToolAncestor(p1, p2) --[[ HasToolAncestor | Line: 29 ]]
	if p2.Parent == nil then
		return false
	end
	return p2.Parent:IsA("Tool") or p1:HasToolAncestor(p2.Parent)
end
function t.IsValidPartToModify(p1, p2) --[[ IsValidPartToModify | Line: 34 ]]
	if not (p2:IsA("BasePart") or p2:IsA("Decal")) then
		return false
	end
	return not p1:HasToolAncestor(p2)
end
function t.CachePartsRecursive(p1, p2) --[[ CachePartsRecursive | Line: 41 ]]
	if not p2 then
		return
	end
...
function t.TeardownTransparency(p1) --[[ TeardownTransparency | Line: 53 ]]
...
function t.SetupTransparency(p1, p2) --[[ SetupTransparency | Line: 79 ]]
	p1:TeardownTransparency()
	if p1.descendantAddedConn then
		p1.descendantAddedConn:disconnect()
	end
	p1.descendantAddedConn = p2.DescendantAdded:Connect(function(p12) --[[ Line: 83 | Upvalues: p1 (copy), p2 (copy) ]]
		...
		p1.toolDescendantAddedConns[p12] = p12.DescendantAdded:Connect(function(p12) --[[ Line: 91 | Upvalues: p1 (ref) ]]
		...
		p1.toolDescendantRemovingConns[p12] = p12.DescendantRemoving:Connect(function(p12) --[[ Line: 99 | Upvalues: p2 (ref), p1 (ref) ]]
			wait()
	...
	p1.descendantRemovingConn = p2.DescendantRemoving:connect(function(p12) --[[ Line: 111 | Upvalues: p1 (copy) ]]
...
function t.Enable(p1, p2) --[[ Enable | Line: 122 ]]
	if p1.enabled == p2 then
		return
	end
	p1.enabled = p2
end
function t.SetSubject(p1, p2) --[[ SetSubject | Line: 128 ]]
...
function t.Update(p1, p2) --[[ Update | Line: 143 | Upvalues: CameraUtils (copy) ]]
