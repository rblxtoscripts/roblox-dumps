...
v1.__index = v1
function v1.new() --[[ new | Line: 42 | Upvalues: BaseCamera (copy), v1 (copy) ]]
...
function v1.LoadOrCreateNumberValueParameter(p1, p2, p3, p4) --[[ LoadOrCreateNumberValueParameter | Line: 81 ]]
	...
	p1.changedSignalConnections[p2] = v1.Changed:Connect(function(p12) --[[ Line: 101 | Upvalues: p1 (copy), p2 (copy), p4 (copy) ]]
		p1.externalProperties[p2] = p12
		p4(p1)
	end)
end
function v1.SetAndBoundsCheckAzimuthValues(p1) --[[ SetAndBoundsCheckAzimuthValues | Line: 108 ]]
...
function v1.SetAndBoundsCheckElevationValues(p1) --[[ SetAndBoundsCheckElevationValues | Line: 118 ]]
...
function v1.SetAndBoundsCheckDistanceValues(p1) --[[ SetAndBoundsCheckDistanceValues | Line: 134 ]]
...
function v1.LoadNumberValueParameters(p1) --[[ LoadNumberValueParameters | Line: 142 ]]
	...
	p1:LoadOrCreateNumberValueParameter("ReferenceAzimuth", "NumberValue", p1.SetAndBoundsCheckAzimuthValue)
...
function v1.GetModuleName(p1) --[[ GetModuleName | Line: 167 ]]
	return "OrbitalCamera"
end
function v1.SetInitialOrientation(p1, p2) --[[ SetInitialOrientation | Line: 171 | Upvalues: CameraUtils (copy) ]]
...
function v1.GetCameraToSubjectDistance(p1) --[[ GetCameraToSubjectDistance | Line: 188 ]]
	return p1.curDistance
end
function v1.SetCameraToSubjectDistance(p1, p2) --[[ SetCameraToSubjectDistance | Line: 192 | Upvalues: Players (copy) ]]
...
function v1.CalculateNewLookVector(p1, p2, p3) --[[ CalculateNewLookVector | Line: 205 ]]
...
function v1.Update(p1, p2) --[[ Update | Line: 216 | Upvalues: CameraInput (copy), Players (copy), VRService (copy) ]]
	local v1 = tick()
	local v2 = v1 - p1.lastUpdate
	local v3 = CameraInput.getRotation() ~= Vector2.new()
	...
	if p1.lastUpdate == nil or v2 > 1 then
	...
	if v8 and (Players.LocalPlayer and CurrentCamera) then
		...
		local v9 = VRService.VREnabled and p1:GetVRFocus(v8, v2) or CFrame.new(v8)
		local v10 = CameraInput.getRotation()
		local p = v9.p
		if VRService.VREnabled and not p1:IsInFirstPerson() then
			local v11 = p1:GetCameraHeight()
			local v12 = v8 - CurrentCamera.CFrame.p
			local magnitude = v12.magnitude
			if p1.currentSubjectDistance < magnitude or v10.x ~= 0 then
				local v13 = math.min(magnitude, p1.currentSubjectDistance)
				local v14 = p1:CalculateNewLookVector(v12.unit * Vector3.new(1, 0, 1), Vector2.new(v10.x, 0)) * v13
				local v15 = p - v14
				local lookVector = CurrentCamera.CFrame.lookVector
				if v10.x ~= 0 then
					lookVector = v14
				end
				local v18 = Vector3.new(v15.x + lookVector.x, v15.y, v15.z + lookVector.z)
				v4 = CFrame.new(v15, v18) + Vector3.new(0, v11, 0)
			end
			Focus = v9
		else
			...
			if p1.useAzimuthLimits then
				...
				if p1.curAzimuthRad == 0 then
					...
					local v20 = math.sign(p1.curAzimuthRad)
					v19 = v20 * (math.abs(p1.curAzimuthRad) % 6.283185307179586)
					if v19 then
						Focus = v9
					else
						v19 = 0
				...
				p1.curAzimuthRad = v19
			end
			p1.curElevationRad = math.clamp(p1.curElevationRad + v10.y, p1.minElevationRad, p1.maxElevationRad)
			v4 = CFrame.new(v8 + p1.currentSubjectDistance * (CFrame.fromEulerAnglesYXZ(-p1.curElevationRad, p1.curAzimuthRad, 0) * Vector3.new(0, 0, 1)), v8)
		end
		p1.lastCameraTransform = v4
