...
local t = {}
t.__index = t
function t.new() --[[ new | Line: 25 | Upvalues: t (copy), GameSettings (copy), Players (copy) ]]
	...
	if BoundKeys then
		BoundKeys.Changed:Connect(function(p1) --[[ Line: 48 | Upvalues: v2 (copy) ]]
			v2:OnBoundKeysObjectChanged(p1)
		end)
		v2:OnBoundKeysObjectChanged(BoundKeys.Value)
	end
	GameSettings.Changed:Connect(function(p1) --[[ Line: 55 | Upvalues: v2 (copy) ]]
		if p1 ~= "ControlMode" and p1 ~= "ComputerMovementMode" then
			return
		end
		v2:UpdateMouseLockAvailability()
	end)
	Players.LocalPlayer:GetPropertyChangedSignal("DevEnableMouseLock"):Connect(function() --[[ Line: 62 | Upvalues: v2 (copy) ]]
		v2:UpdateMouseLockAvailability()
	end)
	Players.LocalPlayer:GetPropertyChangedSignal("DevComputerMovementMode"):Connect(function() --[[ Line: 67 | Upvalues: v2 (copy) ]]
		v2:UpdateMouseLockAvailability()
	end)
	v2:UpdateMouseLockAvailability()
	return v2
end
function t.GetIsMouseLocked(p1) --[[ GetIsMouseLocked | Line: 76 ]]
	return p1.isMouseLocked
end
function t.GetBindableToggleEvent(p1) --[[ GetBindableToggleEvent | Line: 80 ]]
	return p1.mouseLockToggledEvent.Event
end
function t.GetMouseLockOffset(p1) --[[ GetMouseLockOffset | Line: 84 ]]
...
function t.UpdateMouseLockAvailability(p1) --[[ UpdateMouseLockAvailability | Line: 106 | Upvalues: Players (copy), GameSettings (copy) ]]
...
function t.OnBoundKeysObjectChanged(p1, p2) --[[ OnBoundKeysObjectChanged | Line: 118 ]]
	p1.boundKeys = {}
...
function t.OnMouseLockToggled(p1) --[[ OnMouseLockToggled | Line: 133 | Upvalues: CameraUtils (copy) ]]
...
function t.DoMouseLockSwitch(p1, p2, p3, p4) --[[ DoMouseLockSwitch | Line: 157 ]]
...
function t.BindContextActions(p1) --[[ BindContextActions | Line: 165 | Upvalues: ContextActionService (copy), Default (copy) ]]
	ContextActionService:BindActionAtPriority("MouseLockSwitchAction", function(p12, p2, p3) --[[ Line: 166 | Upvalues: p1 (copy) ]]
		return p1:DoMouseLockSwitch(p12, p2, p3)
	end, false, Default, unpack(p1.boundKeys))
end
function t.UnbindContextActions(p1) --[[ UnbindContextActions | Line: 171 | Upvalues: ContextActionService (copy) ]]
	ContextActionService:UnbindAction("MouseLockSwitchAction")
end
function t.IsMouseLocked(p1) --[[ IsMouseLocked | Line: 175 ]]
	return p1.enabled and p1.isMouseLocked
end
function t.EnableMouseLock(p1, p2) --[[ EnableMouseLock | Line: 179 | Upvalues: CameraUtils (copy) ]]
