...
v3.__index = v3
function v3.new() --[[ new | Line: 44 | Upvalues: BaseCharacterController (copy), v3 (copy) ]]
	local v32 = setmetatable(BaseCharacterController.new(), v3)
	v32.moveTouchObject = nil
	v32.moveTouchLockedIn = false
	v32.moveTouchFirstChanged = false
	v32.moveTouchStartPosition = nil
	v32.startImage = nil
	v32.endImage = nil
	v32.middleImages = {}
	v32.startImageFadeTween = nil
	v32.endImageFadeTween = nil
	v32.middleImageFadeTweens = {}
	v32.isFirstTouch = true
	v32.thumbstickFrame = nil
	v32.onRenderSteppedConn = nil
	v32.fadeInAndOutBalance = 0.5
	v32.fadeInAndOutHalfDuration = 0.3
	v32.hasFadedBackgroundInPortrait = false
	v32.hasFadedBackgroundInLandscape = false
	v32.tweenInAlphaStart = nil
	v32.tweenOutAlphaStart = nil
	return v32
end
function v3.GetIsJumping(p1) --[[ GetIsJumping | Line: 79 ]]
	local isJumping = p1.isJumping
	p1.isJumping = false
	return isJumping
end
function v3.Enable(p1, p2, p3) --[[ Enable | Line: 85 | Upvalues: ContextActionService (copy) ]]
	if p2 == nil then
		return false
	end
...
function v3.OnInputEnded(p1) --[[ OnInputEnded | Line: 108 ]]
	p1.moveTouchObject = nil
	p1.moveVector = Vector3.new(0, 0, 0)
	p1:FadeThumbstick(false)
end
function v3.FadeThumbstick(p1, p2) --[[ FadeThumbstick | Line: 114 | Upvalues: TweenService (copy), v2 (copy), t (copy) ]]
...
function v3.FadeThumbstickFrame(p1, p2, p3) --[[ FadeThumbstickFrame | Line: 157 ]]
	p1.fadeInAndOutHalfDuration = p2 * 0.5
	p1.fadeInAndOutBalance = p3
	p1.tweenInAlphaStart = tick()
end
function v3.InputInFrame(p1, p2) --[[ InputInFrame | Line: 163 ]]
...
function v3.DoFadeInBackground(p1) --[[ DoFadeInBackground | Line: 175 | Upvalues: LocalPlayer (ref) ]]
...
local v4 = nil
local function loadSonar() --[[ loadSonar | Line: 199 | Upvalues: v4 (ref) ]]
	v4 = require(game.ReplicatedStorage:WaitForChild("Sonar"))("PlayerWrapper").GetClient()
end
function v3.DoMove(p1, p2) --[[ DoMove | Line: 204 | Upvalues: v4 (ref), loadSonar (copy) ]]
	if not v4 then
		pcall(loadSonar)
	end
	local radiusOfMaxSpeed = p1.radiusOfMaxSpeed
	local v1 = v4 and v4.CurrentHorse
	if v1 and not v1.Flying then
		radiusOfMaxSpeed = radiusOfMaxSpeed * 5
	end
	local v2
	if p2.magnitude < p1.radiusOfDeadZone then
		v2 = Vector3.new(0, 0, 0)
	else
		if v4 and (v4.CanUseMobileHUD and v4.CanUseMobileHUD:get()) then
			radiusOfMaxSpeed = 75
		end
		local v42 = p2.unit * (1 - math.max(0, (radiusOfMaxSpeed - p2.magnitude) / radiusOfMaxSpeed))
		v2 = Vector3.new(v42.x, 0, v42.y)
	end
	p1.moveVector = v2
end
function v3.LayoutMiddleImages(p1, p2, p3) --[[ LayoutMiddleImages | Line: 236 | Upvalues: v1 (copy) ]]
...
function v3.MoveStick(p1, p2) --[[ MoveStick | Line: 267 ]]
...
function v3.BindContextActions(p1) --[[ BindContextActions | Line: 275 | Upvalues: TweenService (copy), ContextActionService (copy), High (copy) ]]
	local function inputBegan(p12) --[[ inputBegan | Line: 276 | Upvalues: p1 (copy), TweenService (ref) ]]
	...
	local function inputChanged(p12) --[[ inputChanged | Line: 308 | Upvalues: p1 (copy) ]]
	...
	local function inputEnded(p12) --[[ inputEnded | Line: 341 | Upvalues: p1 (copy) ]]
		if p12 ~= p1.moveTouchObject then
			return Enum.ContextActionResult.Pass
		end
		p1:OnInputEnded()
	...
	ContextActionService:BindActionAtPriority("DynamicThumbstickAction", function(p12, p2, p3) --[[ handleInput | Line: 351 | Upvalues: inputBegan (copy), inputChanged (copy), p1 (copy) ]]
		if p2 == Enum.UserInputState.Begin then
			return inputBegan(p3)
		end
		if p2 == Enum.UserInputState.Change then
			return inputChanged(p3)
		end
...
function v3.Create(p1, p2) --[[ Create | Line: 371 | Upvalues: v1 (copy), t (copy), RunService (copy), UserInputService (copy), GuiService (copy), LocalPlayer (ref) ]]
	...
	p1.middleSize = 10
	p1.middleSpacing = p1.middleSize + 4
	...
	if math.min(AbsoluteSize.x, AbsoluteSize.y) > 500 then
		p1.thumbstickSize = p1.thumbstickSize * 2
		p1.thumbstickRingSize = p1.thumbstickRingSize * 2
		p1.middleSize = p1.middleSize * 2
		p1.middleSpacing = p1.middleSpacing * 2
		p1.radiusOfDeadZone = p1.radiusOfDeadZone * 2
		p1.radiusOfMaxSpeed = p1.radiusOfMaxSpeed * 2
	end
	local function layoutThumbstickFrame(p12) --[[ layoutThumbstickFrame | Line: 400 | Upvalues: p1 (copy) ]]
		if p12 then
			p1.thumbstickFrame.Size = UDim2.new(1, 0, 0.4, 0)
			p1.thumbstickFrame.Position = UDim2.new(0, 0, 0.6, 0)
		else
			p1.thumbstickFrame.Size = UDim2.new(0.4, 0, 0.6666666666666666, 0)
			p1.thumbstickFrame.Position = UDim2.new(0, 0, 0.3333333333333333, 0)
		end
	end
	...
	p1.startImage.Position = UDim2.new(0, p1.thumbstickRingSize * 3.3, 1, -p1.thumbstickRingSize * 2.8)
	...
	local v2 = nil
	local function onCurrentCameraChanged() --[[ onCurrentCameraChanged | Line: 461 | Upvalues: v2 (ref), layoutThumbstickFrame (copy) ]]
		if v2 then
			v2:Disconnect()
			v2 = nil
		end
		...
		local function onViewportSizeChanged() --[[ onViewportSizeChanged | Line: 468 | Upvalues: CurrentCamera (copy), layoutThumbstickFrame (ref) ]]
	...
	p1.onRenderSteppedConn = RunService.RenderStepped:Connect(function() --[[ Line: 488 | Upvalues: p1 (copy) ]]
	...
	p1.onTouchEndedConn = UserInputService.TouchEnded:connect(function(p12) --[[ Line: 507 | Upvalues: p1 (copy) ]]
		if p12 ~= p1.moveTouchObject then
			return
		end
		p1:OnInputEnded()
	end)
	GuiService.MenuOpened:connect(function() --[[ Line: 513 | Upvalues: p1 (copy) ]]
	...
	local function longShowBackground() --[[ longShowBackground | Line: 529 | Upvalues: p1 (copy) ]]
		p1.fadeInAndOutHalfDuration = 2.5
		p1.fadeInAndOutBalance = 0.05
		p1.tweenInAlphaStart = tick()
	end
	local _ = PlayerGui:GetPropertyChangedSignal("CurrentScreenOrientation"):Connect(function() --[[ Line: 535 | Upvalues: v4 (copy), PlayerGui (ref), v3 (ref), p1 (copy) ]]
	...
	if game:IsLoaded() then
		...
		coroutine.wrap(function() --[[ Line: 555 | Upvalues: p1 (copy) ]]
			game.Loaded:Wait()
			p1.fadeInAndOutHalfDuration = 2.5
			p1.fadeInAndOutBalance = 0.05
			p1.tweenInAlphaStart = tick()
