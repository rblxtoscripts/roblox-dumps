...
local t = {}
local function round(p1) --[[ round | Line: 14 ]]
	return math.floor(p1 + 0.5)
end
local t2 = {}
t2.__index = t2
function t2.new(p1, p2) --[[ new | Line: 23 | Upvalues: t2 (copy) ]]
	return setmetatable({
		vel = 0,
		freq = p1,
		goal = p2,
...
function t2.step(p1, p2) --[[ step | Line: 33 ]]
...
t.Spring = t2
function t.map(p1, p2, p3, p4, p5) --[[ map | Line: 55 ]]
	return (p1 - p2) * (p5 - p4) / (p3 - p2) + p4
end
function t.mapClamp(p1, p2, p3, p4, p5) --[[ mapClamp | Line: 60 ]]
	local v1 = math.min(p4, p5)
	return math.clamp((p1 - p2) * (p5 - p4) / (p3 - p2) + p4, v1, (math.max(p4, p5)))
end
function t.getLooseBoundingSphere(p1) --[[ getLooseBoundingSphere | Line: 69 ]]
...
function t.sanitizeAngle(p1) --[[ sanitizeAngle | Line: 125 ]]
	return (p1 + math.pi) % 6.283185307179586 - math.pi
end
function t.Round(p1, p2) --[[ Round | Line: 130 ]]
	local v1 = 10 ^ p2
	return math.floor(p1 * v1 + 0.5) / v1
end
function t.IsFinite(p1) --[[ IsFinite | Line: 135 ]]
	return if p1 == p1 and p1 ~= (1 / 0) then p1 ~= (-1 / 0) else false
end
function t.IsFiniteVector3(p1) --[[ IsFiniteVector3 | Line: 139 | Upvalues: t (copy) ]]
	return t.IsFinite(p1.X) and (t.IsFinite(p1.Y) and t.IsFinite(p1.Z))
end
function t.GetAngleBetweenXZVectors(p1, p2) --[[ GetAngleBetweenXZVectors | Line: 144 ]]
	return math.atan2(p2.X * p1.Z - p2.Z * p1.X, p2.X * p1.X + p2.Z * p1.Z)
end
function t.RotateVectorByAngleAndRound(p1, p2, p3) --[[ RotateVectorByAngleAndRound | Line: 148 ]]
...
local function SCurveTranform(p1) --[[ SCurveTranform | Line: 162 ]]
...
local function toSCurveSpace(p1) --[[ toSCurveSpace | Line: 171 ]]
	return (math.abs(p1) * 2 - 1) * 1.1 - 0.1
end
local function fromSCurveSpace(p1) --[[ fromSCurveSpace | Line: 175 ]]
	return p1 / 2 + 0.5
end
function t.GamepadLinearToCurve(p1) --[[ GamepadLinearToCurve | Line: 179 ]]
	local function onAxis(p1) --[[ onAxis | Line: 180 ]]
...
function t.ConvertCameraModeEnumToStandard(p1) --[[ ConvertCameraModeEnumToStandard | Line: 193 ]]
...
local function getMouse() --[[ getMouse | Line: 242 | Upvalues: Players (copy) ]]
...
local v1 = ""
local v2 = nil
function t.setMouseIconOverride(p1) --[[ setMouseIconOverride | Line: 254 | Upvalues: Players (copy), v2 (ref), v1 (ref) ]]
	local LocalPlayer = Players.LocalPlayer
	if not LocalPlayer then
		Players:GetPropertyChangedSignal("LocalPlayer"):Wait()
		LocalPlayer = Players.LocalPlayer
...
function t.restoreMouseIcon() --[[ restoreMouseIcon | Line: 265 | Upvalues: Players (copy), v2 (ref), v1 (ref) ]]
	local LocalPlayer = Players.LocalPlayer
	if not LocalPlayer then
		Players:GetPropertyChangedSignal("LocalPlayer"):Wait()
		LocalPlayer = Players.LocalPlayer
...
local v3 = nil
function t.setMouseBehaviorOverride(p1) --[[ setMouseBehaviorOverride | Line: 276 | Upvalues: UserInputService (copy), v3 (ref), Default (ref) ]]
...
function t.restoreMouseBehavior() --[[ restoreMouseBehavior | Line: 285 | Upvalues: UserInputService (copy), v3 (ref), Default (ref) ]]
	if UserInputService.MouseBehavior ~= v3 then
		v3 = nil
		return
	end
...
local v4 = nil
function t.setRotationTypeOverride(p1) --[[ setRotationTypeOverride | Line: 294 | Upvalues: UserGameSettings (copy), v4 (ref), MovementRelative (ref) ]]
...
function t.restoreRotationType() --[[ restoreRotationType | Line: 303 | Upvalues: UserGameSettings (copy), v4 (ref), MovementRelative (ref) ]]
	if UserGameSettings.RotationType == v4 then
