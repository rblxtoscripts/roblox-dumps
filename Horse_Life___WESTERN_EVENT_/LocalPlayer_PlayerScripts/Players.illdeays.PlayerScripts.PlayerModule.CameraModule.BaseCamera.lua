...
local t = {}
t.__index = t
function t.new() --[[ new | Line: 62 | Upvalues: t (copy), LocalPlayer (copy), UserGameSettings (copy) ]]
	...
	LocalPlayer.CharacterAdded:Connect(function(p1) --[[ Line: 130 | Upvalues: v2 (copy) ]]
		v2:OnCharacterAdded(p1)
	end)
	if v2.cameraChangedConn then
		v2.cameraChangedConn:Disconnect()
	end
	v2.cameraChangedConn = workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(function() --[[ Line: 135 | Upvalues: v2 (copy) ]]
	...
	v2.playerCameraModeChangeConn = LocalPlayer:GetPropertyChangedSignal("CameraMode"):Connect(function() --[[ Line: 141 | Upvalues: v2 (copy) ]]
		v2:OnPlayerCameraPropertyChange()
	end)
	if v2.minDistanceChangeConn then
		v2.minDistanceChangeConn:Disconnect()
	end
	v2.minDistanceChangeConn = LocalPlayer:GetPropertyChangedSignal("CameraMinZoomDistance"):Connect(function() --[[ Line: 146 | Upvalues: v2 (copy) ]]
		v2:OnPlayerCameraPropertyChange()
	end)
	if v2.maxDistanceChangeConn then
		v2.maxDistanceChangeConn:Disconnect()
	end
	v2.maxDistanceChangeConn = LocalPlayer:GetPropertyChangedSignal("CameraMaxZoomDistance"):Connect(function() --[[ Line: 151 | Upvalues: v2 (copy) ]]
		v2:OnPlayerCameraPropertyChange()
	end)
	if v2.playerDevTouchMoveModeChangeConn then
		v2.playerDevTouchMoveModeChangeConn:Disconnect()
	end
	v2.playerDevTouchMoveModeChangeConn = LocalPlayer:GetPropertyChangedSignal("DevTouchMovementMode"):Connect(function() --[[ Line: 156 | Upvalues: v2 (copy) ]]
	...
	v2.gameSettingsTouchMoveMoveChangeConn = UserGameSettings:GetPropertyChangedSignal("TouchMovementMode"):Connect(function() --[[ Line: 162 | Upvalues: v2 (copy) ]]
	...
	if not v2.hasGameLoaded then
		v2.gameLoadedConn = game.Loaded:Connect(function() --[[ Line: 172 | Upvalues: v2 (copy) ]]
...
function t.GetModuleName(p1) --[[ GetModuleName | Line: 184 ]]
	return "BaseCamera"
end
function t.OnCharacterAdded(p1, p2) --[[ OnCharacterAdded | Line: 188 | Upvalues: UserInputService (copy), LocalPlayer (copy) ]]
	...
	p2.ChildAdded:Connect(function(p12) --[[ Line: 198 | Upvalues: p1 (copy) ]]
		if not p12:IsA("Tool") then
			return
		end
		p1.isAToolEquipped = true
	end)
	p2.ChildRemoved:Connect(function(p12) --[[ Line: 203 | Upvalues: p1 (copy) ]]
		if not p12:IsA("Tool") then
			return
		end
...
function t.GetHumanoidRootPart(p1) --[[ GetHumanoidRootPart | Line: 211 | Upvalues: LocalPlayer (copy) ]]
...
function t.GetBodyPartToFollow(p1, p2, p3) --[[ GetBodyPartToFollow | Line: 223 ]]
...
function t.GetSubjectCFrame(p1) --[[ GetSubjectCFrame | Line: 235 ]]
...
function t.GetSubjectVelocity(p1) --[[ GetSubjectVelocity | Line: 301 ]]
	local CurrentCamera = workspace.CurrentCamera
	local v1 = if CurrentCamera then CurrentCamera.CameraSubject else CurrentCamera
	if not v1 then
...
function t.GetSubjectRotVelocity(p1) --[[ GetSubjectRotVelocity | Line: 330 ]]
	local CurrentCamera = workspace.CurrentCamera
	local v1 = if CurrentCamera then CurrentCamera.CameraSubject else CurrentCamera
	if not v1 then
		return Vector3.new(0, 0, 0)
...
function t.StepZoom(p1) --[[ StepZoom | Line: 359 | Upvalues: CameraInput (copy), ZoomController (copy) ]]
...
function t.GetSubjectPosition(p1) --[[ GetSubjectPosition | Line: 384 ]]
...
function t.UpdateDefaultSubjectDistance(p1) --[[ UpdateDefaultSubjectDistance | Line: 454 | Upvalues: LocalPlayer (copy) ]]
...
function t.OnViewportSizeChanged(p1) --[[ OnViewportSizeChanged | Line: 462 | Upvalues: UserInputService (copy) ]]
...
function t.OnCurrentCameraChanged(p1) --[[ OnCurrentCameraChanged | Line: 472 | Upvalues: UserInputService (copy) ]]
	if UserInputService.TouchEnabled then
		...
		if CurrentCamera then
			...
			p1.viewportSizeChangedConn = CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(function() --[[ Line: 483 | Upvalues: p1 (copy) ]]
				p1:OnViewportSizeChanged()
			end)
		end
	end
	...
	p1.cameraSubjectChangedConn = CurrentCamera:GetPropertyChangedSignal("CameraSubject"):Connect(function() --[[ Line: 497 | Upvalues: p1 (copy) ]]
		p1:OnNewCameraSubject()
	end)
	p1:OnNewCameraSubject()
end
function t.OnDynamicThumbstickEnabled(p1) --[[ OnDynamicThumbstickEnabled | Line: 504 | Upvalues: UserInputService (copy) ]]
	if not UserInputService.TouchEnabled then
		return
	end
	p1.isDynamicThumbstickEnabled = true
end
function t.OnDynamicThumbstickDisabled(p1) --[[ OnDynamicThumbstickDisabled | Line: 510 ]]
	p1.isDynamicThumbstickEnabled = false
end
function t.OnGameSettingsTouchMovementModeChanged(p1) --[[ OnGameSettingsTouchMovementModeChanged | Line: 514 | Upvalues: LocalPlayer (copy), UserGameSettings (copy) ]]
...
function t.OnDevTouchMovementModeChanged(p1) --[[ OnDevTouchMovementModeChanged | Line: 525 | Upvalues: LocalPlayer (copy) ]]
...
function t.OnPlayerCameraPropertyChange(p1) --[[ OnPlayerCameraPropertyChange | Line: 533 ]]
	p1:SetCameraToSubjectDistance(p1.currentSubjectDistance)
end
function t.InputTranslationToCameraAngleChange(p1, p2, p3) --[[ InputTranslationToCameraAngleChange | Line: 538 ]]
	return p2 * p3
end
function t.GamepadZoomPress(p1) --[[ GamepadZoomPress | Line: 542 ]]
	...
	if v1 > 10 then
		...
		p1:SetCameraToSubjectDistance(40)
	end
end
function t.Enable(p1, p2) --[[ Enable | Line: 554 | Upvalues: CameraInput (copy), LocalPlayer (copy) ]]
	...
	if p1.enabled then
		...
		p1.gamepadZoomPressConnection = CameraInput.gamepadZoomPress:Connect(function() --[[ Line: 560 | Upvalues: p1 (copy) ]]
			p1:GamepadZoomPress()
		end)
...
function t.OnEnable(p1, p2) --[[ OnEnable | Line: 585 ]] end
function t.GetEnabled(p1) --[[ GetEnabled | Line: 589 ]]
	return p1.enabled
end
function t.Cleanup(p1) --[[ Cleanup | Line: 593 | Upvalues: CameraUtils (copy) ]]
...
function t.UpdateMouseBehavior(p1) --[[ UpdateMouseBehavior | Line: 610 | Upvalues: UserGameSettings (copy), CameraUI (copy), CameraInput (copy), CameraToggleStateController (copy), CameraUtils (copy) ]]
...
function t.UpdateForDistancePropertyChange(p1) --[[ UpdateForDistancePropertyChange | Line: 632 ]]
	p1:SetCameraToSubjectDistance(p1.currentSubjectDistance)
end
function t.SetCameraToSubjectDistance(p1, p2) --[[ SetCameraToSubjectDistance | Line: 638 | Upvalues: LocalPlayer (copy), ZoomController (copy) ]]
	local currentSubjectDistance = p1.currentSubjectDistance
	if LocalPlayer.CameraMode == Enum.CameraMode.LockFirstPerson then
		p1.currentSubjectDistance = 0.5
		if not p1.inFirstPerson then
...
function t.SetCameraType(p1, p2) --[[ SetCameraType | Line: 672 ]]
	p1.cameraType = p2
end
function t.GetCameraType(p1) --[[ GetCameraType | Line: 677 ]]
	return p1.cameraType
end
function t.SetCameraMovementMode(p1, p2) --[[ SetCameraMovementMode | Line: 682 ]]
	p1.cameraMovementMode = p2
end
function t.GetCameraMovementMode(p1) --[[ GetCameraMovementMode | Line: 686 ]]
	return p1.cameraMovementMode
end
function t.SetIsMouseLocked(p1, p2) --[[ SetIsMouseLocked | Line: 690 ]]
	p1.inMouseLockedMode = p2
end
function t.GetIsMouseLocked(p1) --[[ GetIsMouseLocked | Line: 694 ]]
	return p1.inMouseLockedMode
end
function t.SetMouseLockOffset(p1, p2) --[[ SetMouseLockOffset | Line: 698 ]]
	p1.mouseLockOffset = p2
end
function t.GetMouseLockOffset(p1) --[[ GetMouseLockOffset | Line: 702 ]]
	return p1.mouseLockOffset
end
function t.InFirstPerson(p1) --[[ InFirstPerson | Line: 706 ]]
	return p1.inFirstPerson
end
function t.EnterFirstPerson(p1) --[[ EnterFirstPerson | Line: 710 ]] end
function t.LeaveFirstPerson(p1) --[[ LeaveFirstPerson | Line: 714 ]] end
function t.GetCameraToSubjectDistance(p1) --[[ GetCameraToSubjectDistance | Line: 719 ]]
	return p1.currentSubjectDistance
end
function t.GetMeasuredDistanceToFocus(p1) --[[ GetMeasuredDistanceToFocus | Line: 726 ]]
	local CurrentCamera = game.Workspace.CurrentCamera
	if CurrentCamera then
...
function t.GetCameraLookVector(p1) --[[ GetCameraLookVector | Line: 734 ]]
...
function t.CalculateNewLookCFrameFromArg(p1, p2, p3) --[[ CalculateNewLookCFrameFromArg | Line: 738 ]]
	local v1 = if p2 then p2 else p1:GetCameraLookVector()
	local v2 = math.asin(v1.y)
	local v3 = math.max(v2 + -1.3962634015954636, v2 + 1.3962634015954636)
	local v5 = math.min(math.min(v2 + -1.3962634015954636, v2 + 1.3962634015954636), v3)
	local v6 = math.max(v5, v3)
	local v9 = Vector2.new(p3.x, (math.clamp(p3.y, v5, if v6 <= v5 then v6 + 0.001 else v6)))
	local v10 = CFrame.new(Vector3.new(0, 0, 0), v1)
	local v11 = CFrame.Angles(0, -v9.x, 0) * v10 * CFrame.Angles(-v9.y, 0, 0)
	_G.LookCF = v11
	return v11
end
function t.CalculateNewLookVectorFromArg(p1, p2, p3) --[[ CalculateNewLookVectorFromArg | Line: 756 ]]
	return p1:CalculateNewLookCFrameFromArg(p2, p3).lookVector
end
function t.CalculateNewLookVectorVRFromArg(p1, p2) --[[ CalculateNewLookVectorVRFromArg | Line: 761 ]]
...
function t.GetHumanoid(p1) --[[ GetHumanoid | Line: 771 | Upvalues: LocalPlayer (copy) ]]
...
function t.GetHumanoidPartToFollow(p1, p2, p3) --[[ GetHumanoidPartToFollow | Line: 789 ]]
...
function t.OnNewCameraSubject(p1) --[[ OnNewCameraSubject | Line: 803 ]]
...
function t.IsInFirstPerson(p1) --[[ IsInFirstPerson | Line: 810 ]]
	return p1.inFirstPerson
end
function t.Update(p1, p2) --[[ Update | Line: 814 ]]
	error("BaseCamera:Update() This is a virtual function that should never be getting called.", 2)
end
function t.GetCameraHeight(p1) --[[ GetCameraHeight | Line: 818 | Upvalues: VRService (copy) ]]
