...
function t2._hasBasePart(p1, p2) --[[ _hasBasePart | Line: 93 ]]
	if #p2:QueryDescendants("BasePart") > 0 then
		return true
	end
...
function t2._waitForStreamedObject(p1, p2, p3) --[[ _waitForStreamedObject | Line: 107 | Upvalues: t (copy) ]]
	...
	if p1:_hasBasePart(p2) then
		...
		local v1 = nil
		v1 = p2.ChildAdded:Connect(function() --[[ Line: 113 | Upvalues: p1 (copy), p2 (copy), v1 (ref), p3 (copy) ]]
			if p1:_hasBasePart(p2) then
				v1:Disconnect()
				p3()
			end
...
function t2.GetClassAddedSignal(p1) --[[ GetClassAddedSignal | Line: 126 | Upvalues: v2 (copy) ]]
...
function t2.GetClassRemovingSignal(p1) --[[ GetClassRemovingSignal | Line: 136 | Upvalues: v2 (copy) ]]
...
function t2.GetTag(p1) --[[ GetTag | Line: 147 ]]
	return p1._tagName
end
function t2.GetAll(p1) --[[ GetAll | Line: 151 ]]
	local t = {}
...
function t2.GetAllSet(p1) --[[ GetAllSet | Line: 160 ]]
	return p1._allClassSet
end
function t2.BindClient(p1, p2) --[[ BindClient | Line: 166 | Upvalues: RunService (copy), CollectionService (copy) ]]
...
function t2.Bind(p1, p2) --[[ Bind | Line: 176 | Upvalues: RunService (copy), CollectionService (copy) ]]
...
function t2.Unbind(p1, p2) --[[ Unbind | Line: 186 | Upvalues: RunService (copy), CollectionService (copy) ]]
...
function t2.UnbindClient(p1, p2) --[[ UnbindClient | Line: 197 | Upvalues: CollectionService (copy) ]]
	assert(if typeof(p2) == "Instance" then true else false)
	CollectionService:RemoveTag(p2, p1._tagName)
end
function t2.Get(p1, p2) --[[ Get | Line: 202 ]]
	assert(if typeof(p2) == "Instance" then true else false, "Argument \'inst\' is not an Instance")
	return p1._instToClass[p2]
end
function t2._add(p1, p2) --[[ _add | Line: 207 ]]
	assert(if typeof(p2) == "Instance" then true else false, "Argument \'inst\' is not an Instance")
			...
			if type(Destroy) ~= "function" then
				...
				if v4 then
					...
					for k, v in pairs(v4) do
						local v6 = v5.Event:Connect(function() --[[ Line: 259 | Upvalues: k (copy), v2 (ref) ]]
							k(v2)
						end)
						v5:Fire()
						v6:Disconnect()
...
function t2._remove(p1, p2) --[[ _remove | Line: 275 ]]
	p1._pendingInstSet[p2] = nil
	...
	if v2 then
		...
		for k, v in pairs(v2) do
			local v4 = v3.Event:Connect(function() --[[ Line: 298 | Upvalues: k (copy) ]]
				k(nil)
			end)
			v3:Fire()
			v4:Disconnect()
