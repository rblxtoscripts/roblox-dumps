...
function t.UpdateShake(p1, p2) --[[ UpdateShake | Line: 63 | Upvalues: noise (copy), v1 (copy) ]]
	...
	if p1.fadeInDuration > 0 and p1.sustain then
		if currentFadeTime < 1 then
			currentFadeTime = currentFadeTime + math.min(p2 / p1.fadeInDuration, p1.fadeInDuration)
		elseif p1.fadeOutDuration > 0 then
			p1.sustain = false
		end
	end
	if not p1.sustain then
		currentFadeTime = currentFadeTime - math.min(p2 / p1.fadeOutDuration, p1.fadeOutDuration)
	end
	if p1.sustain then
