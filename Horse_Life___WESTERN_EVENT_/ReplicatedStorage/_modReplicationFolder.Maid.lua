...
function t.__newindex(p1, p2, p3) --[[ __newindex | Line: 124 | Upvalues: t (copy) ]]
	...
	if typeof(v1) == "table" then
		...
		if typeof(v1) == "thread" then
			if not (if coroutine.running() == v1 then nil else pcall(function() --[[ Line: 151 | Upvalues: v1 (copy) ]]
	task.cancel(v1)
end)) then
				task.defer(function() --[[ Line: 157 | Upvalues: v1 (copy) ]]
					task.cancel(v1)
				end)
			end
		else
...
function t.Add(p1, p2) --[[ Add | Line: 174 ]]
	if not p2 then
...
function t.GiveTask(p1, p2) --[[ GiveTask | Line: 195 ]]
	if not p2 then
		error("Task cannot be false or nil", 2)
	end
	p1[#p1._tasks + 1] = p2
...
function t.GivePromise(p1, p2) --[[ GivePromise | Line: 217 ]]
	return p2
end
function t.DoCleaning(p1) --[[ DoCleaning | Line: 252 ]]
	local _tasks = p1._tasks
			...
			if typeof(v4) == "table" then
				...
				elseif typeof(v4) == "thread" then
					if coroutine.running() == v4 then
						...
						v6 = pcall(function() --[[ Line: 276 | Upvalues: v4 (ref) ]]
							task.cancel(v4)
						end)
						v5 = v6
					end
					if not v5 then
						v7 = v4
						task.defer(function() --[[ Line: 283 | Upvalues: v7 (copy) ]]
							task.cancel(v7)
						end)
					end
