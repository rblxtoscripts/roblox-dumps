...
local v3 = Sonar("LeaderboardService")
...
function t.new(p1) --[[ new | Line: 31 | Upvalues: t (copy), v1 (copy), v8 (copy), v5 (copy), ReplicatedStorage (copy), v6 (copy) ]]
	...
	v2.YourPlacementFrame.Content.UserNameLabel.Text = v8.Player.Name
	v2.YourPlacementFrame.Content.PlayerAvatar.Image = v5.GetUserThumbnail(v8.Player.UserId)
	v2.Type = p1.Parent.Type.Value
	v2.LeaderboardFrame.LoadingLabel.Visible = true
	v2.ReplicateFolder = ReplicatedStorage.Leaderboards:FindFirstChild(v2.Type)
	...
	v2.ScoreValue = v8.PlayerData.Leaderboards[v2.Type].Score
	v2.Maid:GiveTask(v2.ScoreValue:GetPropertyChangedSignal("Value"):Connect(function() --[[ Line: 60 | Upvalues: v2 (copy) ]]
		v2:SetYourScore()
	end))
	v2:SetYourScore()
	...
	v2:SetYourRank()
	v2.Maid:GiveTask(v2.ReplicateFolder.Leaderboard:GetPropertyChangedSignal("Value"):Connect(function() --[[ Line: 80 | Upvalues: v2 (copy) ]]
		v2:Update()
	end))
	v2:Update()
	return v2
...
function t.SetTimer(p1) --[[ SetTimer | Line: 94 | Upvalues: v3 (copy), v4 (copy), RefreshTimer (copy) ]]
	if v3.HasEnded() then
...
function t.SetYourScore(p1) --[[ SetYourScore | Line: 104 | Upvalues: v7 (copy) ]]
	p1.YourPlacementFrame.Content.ScoreLabel.Text = p1.ScoreValue.Value > 0 and v7.splice(p1.ScoreValue.Value) .. " pts" or "N/A"
	if p1.ScoreValue.Value <= 0 then
...
function t.SetYourRank(p1) --[[ SetYourRank | Line: 118 | Upvalues: v8 (copy), t2 (copy), v3 (copy) ]]
	local v1 = p1.PositionsByUserId[v8.Player.UserId]
	local v2 = t2[v1]
...
function t.Update(p1) --[[ Update | Line: 127 | Upvalues: HttpService (copy), t2 (copy), v5 (copy), v8 (copy), v7 (copy) ]]
	local v1 = #p1.ReplicateFolder.Leaderboard.Value > 0 and HttpService:JSONDecode(p1.ReplicateFolder.Leaderboard.Value) or nil
	if v1 and next(v1) then
		...
		for k, v in pairs(v1) do
			...
			v2.Content.UserNameLabel.Text = v.Username or "Loading..."
			v2.Content.PlayerAvatar.Image = v5.GetUserThumbnail(v.UserId)
			v2.Content.UserNameLabel.TextColor3 = v.UserId == v8.Player.UserId and Color3.fromRGB(255, 227, 117) or Color3.fromRGB(255, 255, 255)
			v2.Content.ScoreLabel.TextColor3 = v.UserId == v8.Player.UserId and Color3.fromRGB(255, 227, 117) or Color3.fromRGB(255, 255, 255)
			v2.Content.ScoreLabel.Text = v7.splice((tonumber(v.Amount))) .. " pts"
			v2.Content.RankPlacementLabel.Text = "#" .. k
			v2.Content.RankPlacementLabel.TextColor3 = v4 and v4.Text or (v.UserId == v8.Player.UserId and Color3.fromRGB(255, 227, 117) or Color3.fromRGB(255, 255, 255))
...
function t.Init(p1) --[[ Init | Line: 190 | Upvalues: v2 (copy), t (copy) ]]
	v2.new("Leaderboard", t):Init()
end
t:Init()
return t