...
function t.new() --[[ new | Line: 99 | Upvalues: t (copy), Players (copy), TransparencyController (copy), UserInputService (copy), MouseLockController (copy), UserRespectLegacyCameraOptions (copy), RunService (copy), t2 (copy), t3 (copy), UserGameSettings (copy) ]]
	...
	Players.PlayerAdded:Connect(function(p1) --[[ Line: 120 | Upvalues: v2 (copy) ]]
		v2:OnPlayerAdded(p1)
	end)
	...
	if not UserInputService.TouchEnabled then
		...
		if v3 then
			v3:Connect(function() --[[ Line: 131 | Upvalues: v2 (copy) ]]
				v2:OnMouseLockToggled()
			end)
		end
	end
	if UserRespectLegacyCameraOptions then
		v2:ActivateCameraController()
	else
	...
	RunService:BindToRenderStep("cameraRenderUpdate", Enum.RenderPriority.Camera.Value, function(p1) --[[ Line: 144 | Upvalues: v2 (copy) ]]
		v2:Update(p1)
	end)
	for k, v in pairs(t2) do
		Players.LocalPlayer:GetPropertyChangedSignal(v):Connect(function() --[[ Line: 148 | Upvalues: v2 (copy), v (copy) ]]
			v2:OnLocalPlayerCameraPropertyChanged(v)
		end)
	end
	for k, v in pairs(t3) do
		UserGameSettings:GetPropertyChangedSignal(v):Connect(function() --[[ Line: 154 | Upvalues: v2 (copy), v (copy) ]]
			v2:OnUserGameSettingsPropertyChanged(v)
		end)
	end
	game.Workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(function() --[[ Line: 158 | Upvalues: v2 (copy) ]]
		v2:OnCurrentCameraChanged()
	end)
	return v2
end
function t.GetCameraMovementModeFromSettings(p1) --[[ GetCameraMovementModeFromSettings | Line: 165 | Upvalues: Players (copy), CameraUtils (copy), UserInputService (copy), UserGameSettings (copy) ]]
...
function t.ActivateOcclusionModule(p1, p2) --[[ ActivateOcclusionModule | Line: 190 | Upvalues: Poppercam (copy), Invisicam (copy), t5 (copy), Players (copy) ]]
	local v1
...
function t.ShouldUseVehicleCamera(p1) --[[ ShouldUseVehicleCamera | Line: 269 ]]
...
function t.ActivateCameraController(p1, p2, p3) --[[ ActivateCameraController | Line: 285 | Upvalues: UserRespectLegacyCameraOptions (copy), LegacyCamera (copy), VRService (copy), VRCamera (copy), ClassicCamera (copy), OrbitalCamera (copy), VRVehicleCamera (copy), VehicleCamera (copy), t4 (copy) ]]
	if UserRespectLegacyCameraOptions then
		p3 = workspace.CurrentCamera.CameraType
		p2 = p1:GetCameraMovementModeFromSettings()
	end
	local v2 = nil
	local v3 = if UserRespectLegacyCameraOptions then true elseif p3 == nil then false else true
	if v3 and p3 == Enum.CameraType.Scriptable then
		...
		return
	elseif v3 and p3 == Enum.CameraType.Custom then
		p2 = p1:GetCameraMovementModeFromSettings()
	elseif v3 and p3 == Enum.CameraType.Track then
		p2 = Enum.ComputerCameraMovementMode.Classic
	elseif v3 and p3 == Enum.CameraType.Follow then
		p2 = Enum.ComputerCameraMovementMode.Follow
	elseif v3 and p3 == Enum.CameraType.Orbital then
		p2 = Enum.ComputerCameraMovementMode.Orbital
	elseif v3 and (p3 == Enum.CameraType.Attach or (p3 == Enum.CameraType.Watch or p3 == Enum.CameraType.Fixed)) then
		v2 = LegacyCamera
	elseif v3 then
	...
	if not v2 and VRService.VREnabled then
		v2 = VRCamera
	elseif not v2 and (p2 == Enum.ComputerCameraMovementMode.Classic or (p2 == Enum.ComputerCameraMovementMode.Follow or (p2 == Enum.ComputerCameraMovementMode.Default or p2 == Enum.ComputerCameraMovementMode.CameraToggle))) then
		v2 = ClassicCamera
	elseif not v2 then
...
function t.OnCameraSubjectChanged(p1) --[[ OnCameraSubjectChanged | Line: 397 ]]
	local CurrentCamera = workspace.CurrentCamera
...
function t.OnCameraTypeChanged(p1, p2) --[[ OnCameraTypeChanged | Line: 412 | Upvalues: UserInputService (copy), CameraUtils (copy) ]]
...
function t.OnCurrentCameraChanged(p1) --[[ OnCurrentCameraChanged | Line: 424 ]]
	...
	p1.cameraSubjectChangedConn = CurrentCamera:GetPropertyChangedSignal("CameraSubject"):Connect(function() --[[ Line: 436 | Upvalues: p1 (copy), CurrentCamera (copy) ]]
		p1:OnCameraSubjectChanged(CurrentCamera.CameraSubject)
	end)
	p1.cameraTypeChangedConn = CurrentCamera:GetPropertyChangedSignal("CameraType"):Connect(function() --[[ Line: 440 | Upvalues: p1 (copy), CurrentCamera (copy) ]]
		p1:OnCameraTypeChanged(CurrentCamera.CameraType)
	end)
	p1:OnCameraSubjectChanged(CurrentCamera.CameraSubject)
	p1:OnCameraTypeChanged(CurrentCamera.CameraType)
end
function t.OnLocalPlayerCameraPropertyChanged(p1, p2) --[[ OnLocalPlayerCameraPropertyChanged | Line: 448 | Upvalues: Players (copy), CameraUtils (copy) ]]
...
function t.OnUserGameSettingsPropertyChanged(p1, p2) --[[ OnUserGameSettingsPropertyChanged | Line: 492 | Upvalues: CameraUtils (copy) ]]
	if p2 ~= "ComputerCameraMovementMode" then
		return
	end
	p1:ActivateCameraController(CameraUtils.ConvertCameraModeEnumToStandard((p1:GetCameraMovementModeFromSettings())))
end
function t.Update(p1, p2) --[[ Update | Line: 505 | Upvalues: CameraInput (copy) ]]
	if not p1.activeCameraController then
		return
	end
...
if not UserRespectLegacyCameraOptions then
	function t.GetCameraControlChoice(p1) --[[ GetCameraControlChoice | Line: 534 | Upvalues: Players (copy), UserInputService (copy), CameraUtils (copy), UserGameSettings (copy) ]]
...
function t.OnCharacterAdded(p1, p2, p3) --[[ OnCharacterAdded | Line: 558 ]]
	if not p1.activeOcclusionModule then
		return
	end
	p1.activeOcclusionModule:CharacterAdded(p2, p3)
end
function t.OnCharacterRemoving(p1, p2, p3) --[[ OnCharacterRemoving | Line: 564 ]]
	if not p1.activeOcclusionModule then
		return
	end
	p1.activeOcclusionModule:CharacterRemoving(p2, p3)
end
function t.OnPlayerAdded(p1, p2) --[[ OnPlayerAdded | Line: 570 ]]
	p2.CharacterAdded:Connect(function(p12) --[[ Line: 571 | Upvalues: p1 (copy), p2 (copy) ]]
		p1:OnCharacterAdded(p12, p2)
	end)
	p2.CharacterRemoving:Connect(function(p12) --[[ Line: 574 | Upvalues: p1 (copy), p2 (copy) ]]
		p1:OnCharacterRemoving(p12, p2)
	end)
end
function t.OnMouseLockToggled(p1) --[[ OnMouseLockToggled | Line: 579 ]]
