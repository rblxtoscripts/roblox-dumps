...
local function updatePrice() --[[ updatePrice | Line: 37 | Upvalues: v2 (copy), v1 (copy), uiStyle (copy), coins (copy) ]]
	...
	local v4 = item.Configuration:FindFirstChild("price") or v2.subscription.Value > os.time()
	if method == "product" then
		v1.Visible = not v4
	elseif method == "subscribed" then
		v1.info.yxles.Text = item.Configuration.subscriptionPrice:IsA("ModuleScript") and require(item.Configuration.subscriptionPrice)(v3) or item.Configuration.subscriptionPrice.Value
		if v4 then
		...
		v1.info.yxles.Text = v2:WaitForChild("prices"):WaitForChild(v12):WaitForChild(item.Name).Value
	end
...
local function updateItem() --[[ updateItem | Line: 91 | Upvalues: v1 (copy), v2 (copy), updatePrice (copy), v3 (ref), buy (copy) ]]
	if not script.Parent.item.Value then
		return
	end
	local item = script.Parent.item.Value
	...
	elseif method == "product" then
		...
		v1.info.unsubscribedText:Destroy()
		...
		spawn(function() --[[ Line: 117 | Upvalues: v1 (ref), item (copy) ]]
	...
	v1.InputBegan:Connect(function(p1) --[[ Line: 125 | Upvalues: v3 (ref), v1 (ref), buy (ref) ]]
	...
	v1.InputEnded:Connect(function(p1) --[[ Line: 132 | Upvalues: v3 (ref), v1 (ref) ]]
		if p1.UserInputType ~= Enum.UserInputType.MouseMovement or v3.Value ~= v1 then
			return
		end
		v3.Value = nil
	end)
	v1.TouchTap:Connect(function() --[[ Line: 137 | Upvalues: v1 (ref), v3 (ref), buy (ref) ]]
		if math.floor(v1.info.Position.Y.Scale * 100 + 0.5) == -55 then
			buy()
		else
			v3.Value = v1
		end
...
updateItem()
coins.Changed:Connect(function() --[[ Line: 149 | Upvalues: updatePrice (copy) ]]
	updatePrice()
end)
workspace.ChildAdded:Connect(function(p1) --[[ Line: 150 | Upvalues: updatePrice (copy) ]]
...
game:GetService("RunService").RenderStepped:Connect(function() --[[ Line: 158 | Upvalues: v4 (ref), v2 (copy), updatePrice (copy) ]]
