...
local function pitchVelocity(p1, p2) --[[ pitchVelocity | Line: 26 ]]
	return math.abs((p2.XVector:Dot(p1)))
end
local function yawVelocity(p1, p2) --[[ yawVelocity | Line: 31 ]]
	return math.abs((p2.YVector:Dot(p1)))
end
local v1 = 1 / 60
RunService.Stepped:Connect(function(p1, p2) --[[ Line: 37 | Upvalues: v1 (ref) ]]
	v1 = p2
end)
local v2 = setmetatable({}, BaseCamera)
v2.__index = v2
function v2.new() --[[ new | Line: 44 | Upvalues: BaseCamera (copy), v2 (copy) ]]
	local v3 = setmetatable(BaseCamera.new(), v2)
	v3:Reset()
	return v3
end
function v2.Reset(p1) --[[ Reset | Line: 50 | Upvalues: VehicleCameraCore (copy), Spring (copy), VehicleCameraConfig (copy), CameraUtils (copy) ]]
...
function v2._StepInitialZoom(p1) --[[ _StepInitialZoom | Line: 74 | Upvalues: ZoomController (copy), VehicleCameraConfig (copy) ]]
...
function v2._StepRotation(p1, p2, p3) --[[ _StepRotation | Line: 81 | Upvalues: CameraInput (copy), sanitizeAngle (copy), VehicleCameraConfig (copy), mapClamp (copy) ]]
	local yawSpring = p1.yawSpring
	local pitchSpring = p1.pitchSpring
	local v1 = CameraInput.getRotation(true)
...
function v2._GetThirdPersonLocalOffset(p1) --[[ _GetThirdPersonLocalOffset | Line: 144 | Upvalues: VehicleCameraConfig (copy) ]]
	return p1.assemblyOffset + Vector3.new(0, p1.assemblyRadius * VehicleCameraConfig.verticalCenterOffset, 0)
end
function v2._GetFirstPersonLocalOffset(p1, p2) --[[ _GetFirstPersonLocalOffset | Line: 148 | Upvalues: LocalPlayer (copy) ]]
...
function v2.Update(p1) --[[ Update | Line: 162 | Upvalues: v1 (ref), mapClamp (copy) ]]
	local CurrentCamera = workspace.CurrentCamera
...
function v2.ApplyVRTransform(p1) --[[ ApplyVRTransform | Line: 207 ]] end
function v2.EnterFirstPerson(p1) --[[ EnterFirstPerson | Line: 211 ]]
	p1.inFirstPerson = true
	p1:UpdateMouseBehavior()
end
function v2.LeaveFirstPerson(p1) --[[ LeaveFirstPerson | Line: 216 ]]
