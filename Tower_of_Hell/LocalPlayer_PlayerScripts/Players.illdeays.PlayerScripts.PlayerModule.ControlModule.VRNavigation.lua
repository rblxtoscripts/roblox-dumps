...
local function IsFinite(p1) --[[ IsFinite | Line: 32 ]]
	return if p1 == p1 and p1 ~= (1 / 0) then p1 ~= (-1 / 0) else false
end
local function IsFiniteVector3(p1) --[[ IsFiniteVector3 | Line: 36 ]]
	local x = p1.x
...
coroutine.wrap(function() --[[ Line: 44 | Upvalues: v1 (ref) ]]
...
v2.__index = v2
function v2.new(p1) --[[ new | Line: 57 | Upvalues: BaseCharacterController (copy), v2 (copy) ]]
...
function v2.SetLaserPointerMode(p1, p2) --[[ SetLaserPointerMode | Line: 82 | Upvalues: StarterGui (copy) ]]
	pcall(function() --[[ Line: 83 | Upvalues: StarterGui (ref), p2 (copy) ]]
		StarterGui:SetCore("VRLaserPointerMode", p2)
	end)
end
function v2.GetLocalHumanoid(p1) --[[ GetLocalHumanoid | Line: 88 | Upvalues: LocalPlayer (copy) ]]
...
function v2.HasBothHandControllers(p1) --[[ HasBothHandControllers | Line: 102 | Upvalues: VRService (copy) ]]
...
function v2.HasAnyHandControllers(p1) --[[ HasAnyHandControllers | Line: 106 | Upvalues: VRService (copy) ]]
...
function v2.IsMobileVR(p1) --[[ IsMobileVR | Line: 110 | Upvalues: UserInputService (copy) ]]
	return UserInputService.PreferredInput == Enum.PreferredInput.Touch
end
function v2.HasGamepad(p1) --[[ HasGamepad | Line: 114 | Upvalues: UserInputService (copy) ]]
	return UserInputService.GamepadEnabled
end
function v2.ShouldUseNavigationLaser(p1) --[[ ShouldUseNavigationLaser | Line: 118 ]]
...
function v2.StartFollowingPath(p1, p2) --[[ StartFollowingPath | Line: 140 | Upvalues: MovementUpdate (copy) ]]
	currentPath = p2
...
function v2.GoToPoint(p1, p2) --[[ GoToPoint | Line: 157 | Upvalues: MovementUpdate (copy) ]]
	currentPath = true
	currentPoints = { p2 }
	currentPointIdx = 1
	moving = true
...
function v2.StopFollowingPath(p1) --[[ StopFollowingPath | Line: 173 ]]
	currentPath = nil
	currentPoints = nil
	currentPointIdx = 0
	moving = false
...
function v2.TryComputePath(p1, p2, p3) --[[ TryComputePath | Line: 181 | Upvalues: PathfindingService (copy) ]]
	local v1 = nil
	local count = 0
...
function v2.OnNavigationRequest(p1, p2, p3) --[[ OnNavigationRequest | Line: 208 | Upvalues: v1 (ref) ]]
	...
	local Position_2 = v2.Torso.Position
	if (p1.currentDestination - Position_2).magnitude < 12 then
	...
	if currentDestination and not ((p1.currentDestination - currentDestination).magnitude > 4) then
		...
		local v3 = p1:TryComputePath(Position_2, p1.currentDestination)
		if v3 then
...
function v2.OnJumpAction(p1, p2, p3, p4) --[[ OnJumpAction | Line: 254 ]]
...
function v2.BindJumpAction(p1, p2) --[[ BindJumpAction | Line: 260 | Upvalues: ContextActionService (copy) ]]
	if p2 then
		if not p1.isJumpBound then
			p1.isJumpBound = true
			ContextActionService:BindActionAtPriority("VRJumpAction", function() --[[ Line: 264 | Upvalues: p1 (copy) ]]
...
function v2.ControlCharacterGamepad(p1, p2, p3, p4) --[[ ControlCharacterGamepad | Line: 275 | Upvalues: v1 (ref), MovementUpdate (copy) ]]
...
function v2.OnHeartbeat(p1, p2) --[[ OnHeartbeat | Line: 318 | Upvalues: v1 (ref), MovementUpdate (copy) ]]
	...
	if p1.moving and p1.currentPoints then
		...
		local v2 = (currentPoints[1] - Position) * Vector3.new(1, 0, 1)
		...
		if magnitude < 1 then
			...
			local v6, v7, _ = workspace:FindPartOnRayWithIgnoreList(Ray.new(Position - Vector3.new(0, 1, 0), v3 * 3), t)
			if v6 then
				local _2, v9, _3 = workspace:FindPartOnRayWithIgnoreList(Ray.new(v7 + v3 * 0.5 + Vector3.new(0, 100, 0), Vector3.new(-0, -100, -0)), t)
				local v10 = v9.Y - Position.Y
				if v10 < 6 and v10 > -2 then
					v12.Jump = true
				end
			end
			...
			moveVector = p1.moveVector:Lerp(v3, p2 * 10)
		end
	end
	local x = moveVector.x
	local v122 = if x == x and x ~= (1 / 0) then x ~= (-1 / 0) else false
	if v122 then
		local y = moveVector.y
		v122 = if y == y and y ~= (1 / 0) then y ~= (-1 / 0) else false
		if v122 then
			local z = moveVector.z
			v122 = if z == z and z ~= (1 / 0) then if z == (-1 / 0) then false else true else false
		end
	end
	if not v122 then
		return
...
function v2.OnUserCFrameEnabled(p1) --[[ OnUserCFrameEnabled | Line: 402 ]]
	if p1:ShouldUseNavigationLaser() then
		p1:BindJumpAction(false)
		p1:SetLaserPointerMode("Navigation")
	else
...
function v2.Enable(p1, p2) --[[ Enable | Line: 412 | Upvalues: VRService (copy), RunService (copy), ContextActionService (copy) ]]
	p1.moveVector = Vector3.new(0, 0, 0)
	p1.isJumping = false
	if p2 then
		p1.navigationRequestedConn = VRService.NavigationRequested:Connect(function(p12, p2) --[[ Line: 418 | Upvalues: p1 (copy) ]]
			p1:OnNavigationRequest(p12, p2)
		end)
		p1.heartbeatConn = RunService.Heartbeat:Connect(function(p12) --[[ Line: 419 | Upvalues: p1 (copy) ]]
			p1:OnHeartbeat(p12)
		end)
		ContextActionService:BindAction("MoveThumbstick", function(p12, p2, p3) --[[ Line: 421 | Upvalues: p1 (copy) ]]
			return p1:ControlCharacterGamepad(p12, p2, p3)
		end, false, p1.CONTROL_ACTION_PRIORITY, Enum.KeyCode.Thumbstick1)
		ContextActionService:BindActivate(Enum.UserInputType.Gamepad1, Enum.KeyCode.ButtonR2)
		p1.userCFrameEnabledConn = VRService.UserCFrameEnabled:Connect(function() --[[ Line: 425 | Upvalues: p1 (copy) ]]
