...
function t.new() --[[ new | Line: 95 | Upvalues: t (copy), Players (copy), TransparencyController (copy), UserInputService (copy), MouseLockController (copy), RunService (copy), t2 (copy), t3 (copy), UserGameSettings (copy) ]]
	...
	Players.PlayerAdded:Connect(function(p1) --[[ Line: 116 | Upvalues: v2 (copy) ]]
		v2:OnPlayerAdded(p1)
	end)
	...
	if UserInputService.PreferredInput ~= Enum.PreferredInput.Touch then
		...
		if v3 then
			v3:Connect(function() --[[ Line: 127 | Upvalues: v2 (copy) ]]
	...
	RunService:BindToRenderStep("cameraRenderUpdate", Enum.RenderPriority.Camera.Value, function(p1) --[[ Line: 136 | Upvalues: v2 (copy) ]]
		v2:Update(p1)
	end)
	for k, v in pairs(t2) do
		Players.LocalPlayer:GetPropertyChangedSignal(v):Connect(function() --[[ Line: 140 | Upvalues: v2 (copy), v (copy) ]]
			v2:OnLocalPlayerCameraPropertyChanged(v)
		end)
	end
	for k, v in pairs(t3) do
		UserGameSettings:GetPropertyChangedSignal(v):Connect(function() --[[ Line: 146 | Upvalues: v2 (copy), v (copy) ]]
			v2:OnUserGameSettingsPropertyChanged(v)
		end)
	end
	game.Workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(function() --[[ Line: 150 | Upvalues: v2 (copy) ]]
		v2:OnCurrentCameraChanged()
	end)
	return v2
end
function t.GetCameraMovementModeFromSettings(p1) --[[ GetCameraMovementModeFromSettings | Line: 157 | Upvalues: Players (copy), CameraUtils (copy), UserInputService (copy), UserGameSettings (copy) ]]
...
function t.ActivateOcclusionModule(p1, p2) --[[ ActivateOcclusionModule | Line: 182 | Upvalues: Poppercam (copy), Invisicam (copy), t5 (copy), Players (copy) ]]
	local v1
...
function t.ShouldUseVehicleCamera(p1) --[[ ShouldUseVehicleCamera | Line: 261 ]]
...
function t.ActivateCameraController(p1, p2, p3) --[[ ActivateCameraController | Line: 279 | Upvalues: LegacyCamera (copy), VRService (copy), VRCamera (copy), ClassicCamera (copy), OrbitalCamera (copy), VRVehicleCamera (copy), VehicleCamera (copy), t4 (copy) ]]
	local v1 = nil
...
function t.OnCameraSubjectChanged(p1) --[[ OnCameraSubjectChanged | Line: 384 ]]
	local CurrentCamera = workspace.CurrentCamera
...
function t.OnCameraTypeChanged(p1, p2) --[[ OnCameraTypeChanged | Line: 399 | Upvalues: UserInputService (copy), CameraUtils (copy) ]]
...
function t.OnCurrentCameraChanged(p1) --[[ OnCurrentCameraChanged | Line: 411 ]]
	...
	p1.cameraSubjectChangedConn = CurrentCamera:GetPropertyChangedSignal("CameraSubject"):Connect(function() --[[ Line: 423 | Upvalues: p1 (copy), CurrentCamera (copy) ]]
		p1:OnCameraSubjectChanged(CurrentCamera.CameraSubject)
	end)
	p1.cameraTypeChangedConn = CurrentCamera:GetPropertyChangedSignal("CameraType"):Connect(function() --[[ Line: 427 | Upvalues: p1 (copy), CurrentCamera (copy) ]]
		p1:OnCameraTypeChanged(CurrentCamera.CameraType)
	end)
	p1:OnCameraSubjectChanged(CurrentCamera.CameraSubject)
	p1:OnCameraTypeChanged(CurrentCamera.CameraType)
end
function t.OnLocalPlayerCameraPropertyChanged(p1, p2) --[[ OnLocalPlayerCameraPropertyChanged | Line: 435 | Upvalues: Players (copy), CameraUtils (copy) ]]
...
function t.OnUserGameSettingsPropertyChanged(p1, p2) --[[ OnUserGameSettingsPropertyChanged | Line: 479 | Upvalues: CameraUtils (copy) ]]
	if p2 ~= "ComputerCameraMovementMode" then
		return
	end
	p1:ActivateCameraController(CameraUtils.ConvertCameraModeEnumToStandard((p1:GetCameraMovementModeFromSettings())))
end
function t.Update(p1, p2) --[[ Update | Line: 492 | Upvalues: CameraInput (copy) ]]
	if not p1.activeCameraController then
		return
	end
...
function t.GetCameraControlChoice(p1) --[[ GetCameraControlChoice | Line: 520 | Upvalues: Players (copy), UserInputService (copy), CameraUtils (copy), UserGameSettings (copy) ]]
	local LocalPlayer = Players.LocalPlayer
	if not LocalPlayer then
		return
	end
	if UserInputService:GetLastInputType() == Enum.UserInputType.Touch or UserInputService.PreferredInput == Enum.PreferredInput.Touch then
...
function t.OnCharacterAdded(p1, p2, p3) --[[ OnCharacterAdded | Line: 543 ]]
	if not p1.activeOcclusionModule then
		return
	end
	p1.activeOcclusionModule:CharacterAdded(p2, p3)
end
function t.OnCharacterRemoving(p1, p2, p3) --[[ OnCharacterRemoving | Line: 549 ]]
	if not p1.activeOcclusionModule then
		return
	end
	p1.activeOcclusionModule:CharacterRemoving(p2, p3)
end
function t.OnPlayerAdded(p1, p2) --[[ OnPlayerAdded | Line: 555 ]]
	p2.CharacterAdded:Connect(function(p12) --[[ Line: 556 | Upvalues: p1 (copy), p2 (copy) ]]
		p1:OnCharacterAdded(p12, p2)
	end)
	p2.CharacterRemoving:Connect(function(p12) --[[ Line: 559 | Upvalues: p1 (copy), p2 (copy) ]]
		p1:OnCharacterRemoving(p12, p2)
	end)
end
function t.OnMouseLockToggled(p1) --[[ OnMouseLockToggled | Line: 564 ]]
