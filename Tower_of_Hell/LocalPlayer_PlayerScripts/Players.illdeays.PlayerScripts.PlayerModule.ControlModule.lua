...
local v1, v2 = pcall(function() --[[ Line: 32 ]]
...
local v4 = nil
function t.new() --[[ new | Line: 89 | Upvalues: t (copy), Players (copy), v3 (ref), GuiService (copy), VehicleController (copy), Default (copy), RunService (copy), UserInputService (copy), UserGameSettings (copy) ]]
	...
	if v3 then
		GuiService.MenuOpened:Connect(function() --[[ Line: 111 | Upvalues: v2 (copy) ]]
			if not (v2.touchControlFrame and v2.touchControlFrame.Visible) then
				return
			end
			v2.touchControlFrame.Visible = false
		end)
		GuiService.MenuClosed:Connect(function() --[[ Line: 117 | Upvalues: v2 (copy) ]]
	...
	Players.LocalPlayer.CharacterAdded:Connect(function(p1) --[[ Line: 126 | Upvalues: v2 (copy) ]]
		v2:OnCharacterAdded(p1)
	end)
	Players.LocalPlayer.CharacterRemoving:Connect(function(p1) --[[ Line: 127 | Upvalues: v2 (copy) ]]
		v2:OnCharacterRemoving(p1)
	end)
	if Players.LocalPlayer.Character then
		v2:OnCharacterAdded(Players.LocalPlayer.Character)
	end
	RunService:BindToRenderStep("ControlScriptRenderstep", Enum.RenderPriority.Input.Value, function(p1) --[[ Line: 132 | Upvalues: v2 (copy) ]]
		v2:OnRenderStepped(p1)
	end)
	UserInputService.LastInputTypeChanged:Connect(function(p1) --[[ Line: 136 | Upvalues: v2 (copy) ]]
		v2:OnLastInputTypeChanged(p1)
	end)
	UserGameSettings:GetPropertyChangedSignal("TouchMovementMode"):Connect(function() --[[ Line: 141 | Upvalues: v2 (copy) ]]
		v2:OnTouchMovementModeChange()
	end)
	Players.LocalPlayer:GetPropertyChangedSignal("DevTouchMovementMode"):Connect(function() --[[ Line: 144 | Upvalues: v2 (copy) ]]
		v2:OnTouchMovementModeChange()
	end)
	UserGameSettings:GetPropertyChangedSignal("ComputerMovementMode"):Connect(function() --[[ Line: 148 | Upvalues: v2 (copy) ]]
		v2:OnComputerMovementModeChange()
	end)
	Players.LocalPlayer:GetPropertyChangedSignal("DevComputerMovementMode"):Connect(function() --[[ Line: 151 | Upvalues: v2 (copy) ]]
		v2:OnComputerMovementModeChange()
	end)
	v2.playerGui = nil
	v2.touchGui = nil
	v2.playerGuiAddedConn = nil
	GuiService:GetPropertyChangedSignal("TouchControlsEnabled"):Connect(function() --[[ Line: 160 | Upvalues: v2 (copy) ]]
	...
	if v2.playerGui then
		...
		v2.playerGuiAddedConn = Players.LocalPlayer.ChildAdded:Connect(function(p1) --[[ Line: 171 | Upvalues: v2 (copy), UserInputService (ref) ]]
...
function t.GetMoveVector(p1) --[[ GetMoveVector | Line: 191 ]]
...
function t.GetActiveController(p1) --[[ GetActiveController | Line: 198 ]]
	return p1.activeController
end
function t.UpdateActiveControlModuleEnabled(p1) --[[ UpdateActiveControlModuleEnabled | Line: 203 | Upvalues: Players (copy), ClickToMoveController (copy), GuiService (copy), UserInputService (copy), TouchThumbstick (copy), DynamicThumbstick (copy) ]]
	local function f1() --[[ Line: 213 | Upvalues: p1 (copy), ClickToMoveController (ref), Players (ref) ]]
...
function t.Enable(p1, p2) --[[ Enable | Line: 253 ]]
	if p2 == nil then
		p2 = true
	end
...
function t.Disable(p1) --[[ Disable | Line: 267 ]]
	p1.controlsEnabled = false
	p1:UpdateActiveControlModuleEnabled()
end
function t.SelectComputerMovementModule(p1) --[[ SelectComputerMovementModule | Line: 275 | Upvalues: UserInputService (copy), Players (copy), t3 (copy), v4 (ref), UserGameSettings (copy), Keyboard (copy), ClickToMoveController (copy), t2 (copy) ]]
...
function t.SelectTouchModule(p1) --[[ SelectTouchModule | Line: 313 | Upvalues: UserInputService (copy), Players (copy), t2 (copy), UserGameSettings (copy) ]]
	if UserInputService.PreferredInput ~= Enum.PreferredInput.Touch then
		return nil, false
	end
...
local function calculateRawMoveVector(p1, p2) --[[ calculateRawMoveVector | Line: 329 | Upvalues: Workspace (copy), VRService (copy) ]]
...
function t.OnRenderStepped(p1, p2) --[[ OnRenderStepped | Line: 370 | Upvalues: Gamepad (copy), calculateRawMoveVector (copy), Players (copy) ]]
...
function t.OnHumanoidSeated(p1, p2, p3) --[[ OnHumanoidSeated | Line: 413 | Upvalues: Default (copy) ]]
	if p2 then
...
function t.OnCharacterAdded(p1, p2) --[[ OnCharacterAdded | Line: 428 ]]
	...
	p1.humanoidSeatedConn = p1.humanoid.Seated:Connect(function(p12, p2) --[[ Line: 441 | Upvalues: p1 (copy) ]]
		p1:OnHumanoidSeated(p12, p2)
	end)
end
function t.OnCharacterRemoving(p1, p2) --[[ OnCharacterRemoving | Line: 446 ]]
	p1.humanoid = nil
	p1:UpdateTouchGuiVisibility()
end
function t.UpdateTouchGuiVisibility(p1) --[[ UpdateTouchGuiVisibility | Line: 452 | Upvalues: GuiService (copy) ]]
	if not p1.touchGui then
		return
	end
	p1.touchGui.Enabled = p1.humanoid and GuiService.TouchControlsEnabled and true or false
end
function t.SwitchToController(p1, p2) --[[ SwitchToController | Line: 466 | Upvalues: Default (copy), ClickToMoveController (copy), TouchThumbstick (copy), DynamicThumbstick (copy), TouchJump (copy) ]]
	if p2 then
...
function t.OnLastInputTypeChanged(p1, p2) --[[ OnLastInputTypeChanged | Line: 508 | Upvalues: v4 (ref), t3 (copy) ]]
	if v4 == p2 then
...
function t.OnComputerMovementModeChange(p1) --[[ OnComputerMovementModeChange | Line: 535 ]]
...
function t.OnTouchMovementModeChange(p1) --[[ OnTouchMovementModeChange | Line: 542 ]]
	local v1, v2 = p1:SelectTouchModule()
	if not v2 then
		return
	end
...
function t.CreateTouchGuiContainer(p1) --[[ CreateTouchGuiContainer | Line: 552 ]]
	if p1.touchGui then
		p1.touchGui:Destroy()
	end
...
function t.GetClickToMoveController(p1) --[[ GetClickToMoveController | Line: 571 | Upvalues: ClickToMoveController (copy), Default (copy) ]]
