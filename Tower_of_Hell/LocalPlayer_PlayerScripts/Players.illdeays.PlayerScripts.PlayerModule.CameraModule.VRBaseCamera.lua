...
local v1, v2 = pcall(function() --[[ Line: 29 ]]
...
v4.__index = v4
function v4.new() --[[ new | Line: 40 | Upvalues: BaseCamera (copy), v4 (copy), LocalPlayer (copy) ]]
...
function v4.GetModuleName(p1) --[[ GetModuleName | Line: 59 ]]
	return "VRBaseCamera"
end
function v4.GamepadZoomPress(p1) --[[ GamepadZoomPress | Line: 63 ]]
...
function v4.GamepadReset(p1) --[[ GamepadReset | Line: 78 ]]
	p1.needsReset = true
end
function v4.ResetZoom(p1) --[[ ResetZoom | Line: 82 | Upvalues: ZoomController (copy) ]]
	ZoomController.SetZoomParameters(p1.currentSubjectDistance, 0)
	ZoomController.ReleaseSpring()
end
function v4.OnEnable(p1, p2) --[[ OnEnable | Line: 87 | Upvalues: CameraInput (copy), LocalPlayer (copy), Lighting (copy) ]]
	if p2 then
		p1.gamepadResetConnection = CameraInput.gamepadReset:Connect(function() --[[ Line: 89 | Upvalues: p1 (copy) ]]
			p1:GamepadReset()
		end)
		return
	end
...
function v4.UpdateDefaultSubjectDistance(p1) --[[ UpdateDefaultSubjectDistance | Line: 113 | Upvalues: LocalPlayer (copy) ]]
	p1.defaultSubjectDistance = math.clamp(7, LocalPlayer.CameraMinZoomDistance, LocalPlayer.CameraMaxZoomDistance)
end
function v4.GetCameraToSubjectDistance(p1) --[[ GetCameraToSubjectDistance | Line: 118 ]]
	return p1.currentSubjectDistance
end
function v4.SetCameraToSubjectDistance(p1, p2) --[[ SetCameraToSubjectDistance | Line: 123 | Upvalues: LocalPlayer (copy), ZoomController (copy) ]]
...
function v4.GetVRFocus(p1, p2, p3) --[[ GetVRFocus | Line: 147 ]]
...
function v4.StartFadeFromBlack(p1) --[[ StartFadeFromBlack | Line: 163 | Upvalues: UserGameSettings (copy), Lighting (copy) ]]
...
function v4.UpdateFadeFromBlack(p1, p2) --[[ UpdateFadeFromBlack | Line: 178 | Upvalues: Lighting (copy) ]]
	local VRFade = Lighting:FindFirstChild("VRFade")
...
function v4.StartVREdgeBlur(p1, p2) --[[ StartVREdgeBlur | Line: 194 | Upvalues: UserGameSettings (copy), RunService (copy), VRService (copy), v3 (ref) ]]
	if UserGameSettings.VignetteEnabled == false then
		return
	end
	...
	if not VRBlurPart then
		...
		RunService.RenderStepped:Connect(function(p1) --[[ Line: 214 | Upvalues: VRService (ref), v3 (ref), VRBlurPart (ref) ]]
			...
			if v3 then
				...
				local v32 = workspace.Camera.CFrame * v1
				VRBlurPart.CFrame = v32 * CFrame.Angles(0, math.pi, 0) + v32.LookVector * 1.05
			end
		end)
	end
	...
	if not v1 then
		...
		local v32 = workspace.CurrentCamera.ViewportSize.X * 2.3 / 512
		VRBlur.Size = UDim2.fromScale(v32, workspace.CurrentCamera.ViewportSize.Y * 2.3 / 512)
...
function v4.UpdateEdgeBlur(p1, p2, p3) --[[ UpdateEdgeBlur | Line: 268 ]]
	local VRBlurScreen = p2.PlayerGui:FindFirstChild("VRBlurScreen")
	local v1 = if VRBlurScreen then VRBlurScreen:FindFirstChild("VRBlur") else nil
	if not v1 then
		return
...
function v4.GetCameraHeight(p1) --[[ GetCameraHeight | Line: 293 ]]
	if p1.inFirstPerson then
		return 0
	else
...
function v4.GetSubjectCFrame(p1) --[[ GetSubjectCFrame | Line: 300 | Upvalues: BaseCamera (copy) ]]
...
function v4.GetSubjectPosition(p1) --[[ GetSubjectPosition | Line: 326 | Upvalues: BaseCamera (copy) ]]
