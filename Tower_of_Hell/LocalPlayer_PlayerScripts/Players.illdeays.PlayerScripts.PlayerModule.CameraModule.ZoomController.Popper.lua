...
local t = {}
local tbl = {}
local function refreshIgnoreList() --[[ refreshIgnoreList | Line: 51 | Upvalues: t (ref), tbl (copy) ]]
	local count = 1
	t = {}
...
local function playerAdded(p1) --[[ playerAdded | Line: 60 | Upvalues: tbl (copy), t (ref) ]]
	local function characterAdded(p12) --[[ characterAdded | Line: 61 | Upvalues: tbl (ref), p1 (copy), t (ref) ]]
		tbl[p1] = p12
		local count = 1
		t = {}
		for k, v in pairs(tbl) do
	...
	local function characterRemoving() --[[ characterRemoving | Line: 65 | Upvalues: tbl (ref), p1 (copy), t (ref) ]]
		tbl[p1] = nil
		local count = 1
		t = {}
		for k, v in pairs(tbl) do
...
local function playerRemoving(p1) --[[ playerRemoving | Line: 77 | Upvalues: tbl (copy), t (ref) ]]
	tbl[p1] = nil
	local count = 1
	t = {}
	for k, v in pairs(tbl) do
...
local v6 = nil
local v7 = nil
CurrentCamera:GetPropertyChangedSignal("CameraSubject"):Connect(function() --[[ Line: 111 | Upvalues: CurrentCamera (copy), v7 (ref) ]]
...
local function canOcclude(p1) --[[ canOcclude | Line: 122 | Upvalues: v6 (ref) ]]
	return 1 - (1 - p1.Transparency) * (1 - p1.LocalTransparencyModifier) < 0.25 and (p1.CanCollide and v6 ~= (p1:GetRootPart() or p1)) and not p1:IsA("TrussPart") or p1.Name == "occlude"
end
local t2 = {
	Vector2.new(0.4, 0),
	Vector2.new(-0.4, 0),
...
local function getCollisionPoint(p1, p2) --[[ getCollisionPoint | Line: 151 | Upvalues: t (ref), v1 (copy) ]]
	local v12 = #t
	while true do
...
local function queryPoint(p1, p2, p3, p4) --[[ queryPoint | Line: 173 | Upvalues: t (ref), NearPlaneZ (ref), v1 (copy), v6 (ref) ]]
	...
	repeat
		...
		if v8 then
			...
			if 1 - (1 - v8.Transparency) * (1 - v8.LocalTransparencyModifier) < 0.25 and (v8.CanCollide and v6 ~= (v8:GetRootPart() or v8)) and not v8:IsA("TrussPart") or (if v8.Name == "occlude" then true else false) or v10 then
				local t2 = { v8 }
...
local function queryViewport(p1, p2) --[[ queryViewport | Line: 233 | Upvalues: CurrentCamera (copy), v2 (ref), v3 (ref), NearPlaneZ (ref), queryPoint (copy) ]]
...
local function testPromotion(p1, p2, p3) --[[ testPromotion | Line: 274 | Upvalues: getCollisionPoint (copy), min (copy), queryPoint (copy), t2 (copy) ]]
...
return function(p1, p2, p3) --[[ Popper | Line: 323 | Upvalues: v6 (ref), v7 (ref), queryViewport (copy), testPromotion (copy) ]]
