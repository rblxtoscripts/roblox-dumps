...
local function AssertTypes(p1, ...) --[[ AssertTypes | Line: 59 ]]
	local t = {}
	local v1 = ""
	for k, v in pairs({ ... }) do
		t[v] = true
...
local function Det3x3(p1, p2, p3, p4, p5, p6, p7, p8, p9) --[[ Det3x3 | Line: 71 ]]
	return p1 * (p5 * p9 - p6 * p8) - p2 * (p4 * p9 - p6 * p7) + p3 * (p4 * p8 - p5 * p7)
end
local function RayIntersection(p1, p2, p3, p4) --[[ RayIntersection | Line: 79 ]]
...
v1.__index = v1
function v1.new() --[[ new | Line: 112 | Upvalues: BaseOcclusion (copy), v1 (copy), t (copy) ]]
...
function v1.Enable(p1, p2) --[[ Enable | Line: 145 ]]
	p1.enabled = p2
	if p2 then
		return
	end
...
function v1.GetOcclusionMode(p1) --[[ GetOcclusionMode | Line: 153 ]]
	return Enum.DevCameraOcclusionMode.Invisicam
end
function v1.LimbBehavior(p1, p2) --[[ LimbBehavior | Line: 158 ]]
	for k, v in pairs(p1.trackedLimbs) do
		p2[#p2 + 1] = k.Position
	end
end
function v1.MoveBehavior(p1, p2) --[[ MoveBehavior | Line: 164 ]]
	for i = 1, 3 do
...
function v1.CornerBehavior(p1, p2) --[[ CornerBehavior | Line: 173 | Upvalues: t3 (copy) ]]
...
function v1.CircleBehavior(p1, p2) --[[ CircleBehavior | Line: 184 | Upvalues: t (copy) ]]
	local v1
...
function v1.LimbMoveBehavior(p1, p2) --[[ LimbMoveBehavior | Line: 200 ]]
	p1:LimbBehavior(p2)
	p1:MoveBehavior(p2)
end
function v1.CharacterOutlineBehavior(p1, p2) --[[ CharacterOutlineBehavior | Line: 205 ]]
	...
	for i = 1, 24 do
		...
		local v11, v12 = game.Workspace:FindPartOnRayWithWhitelist(Ray.new(v3 + v9, -3 * v9), t, false)
		if v11 then
			p2[#p2 + 1] = v12 + 0.2 * (v3 - v12).unit
		end
	end
end
function v1.SmartCircleBehavior(p1, p2) --[[ SmartCircleBehavior | Line: 244 | Upvalues: RayIntersection (copy) ]]
	local unit = p1.torsoPart.CFrame.upVector.unit
	local unit_2 = p1.torsoPart.CFrame.rightVector.unit
	p2[#p2 + 1] = p1.torsoPart.CFrame.p
	p2[#p2 + 1] = p1.torsoPart.CFrame.p + unit
...
function v1.CheckTorsoReference(p1) --[[ CheckTorsoReference | Line: 329 ]]
	if not p1.char then
		return
	end
...
function v1.CharacterAdded(p1, p2, p3) --[[ CharacterAdded | Line: 343 | Upvalues: Players (copy), t2 (copy) ]]
	...
	p1.char = p2
	p1.trackedLimbs = {}
	local function childAdded(p12) --[[ childAdded | Line: 359 | Upvalues: t2 (ref), p1 (copy) ]]
	...
	local function childRemoved(p12) --[[ childRemoved | Line: 375 | Upvalues: p1 (copy) ]]
...
function v1.SetMode(p1, p2) --[[ SetMode | Line: 389 | Upvalues: AssertTypes (copy), t (copy) ]]
...
function v1.GetObscuredParts(p1) --[[ GetObscuredParts | Line: 401 ]]
	return p1.savedHits
end
function v1.Cleanup(p1) --[[ Cleanup | Line: 406 ]]
	for k, v in pairs(p1.savedHits) do
		k.LocalTransparencyModifier = v
	end
end
function v1.Update(p1, p2, p3, p4) --[[ Update | Line: 412 ]]
	if p1.enabled and p1.char then
		...
		if not p1.humanoidRootPart then
			...
			local v1 = nil
			v1 = p1.humanoidRootPart.AncestryChanged:Connect(function(p12, p2) --[[ Line: 437 | Upvalues: p1 (copy), v1 (ref) ]]
		...
		local t2 = {}
		local t3 = { p1.char }
		local function add(p12) --[[ add | Line: 463 | Upvalues: t2 (copy), p1 (copy) ]]
			t2[p12] = true
