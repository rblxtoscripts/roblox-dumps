...
local function pitchVelocity(p1, p2) --[[ pitchVelocity | Line: 35 ]]
	return math.abs((p2.XVector:Dot(p1)))
end
local function yawVelocity(p1, p2) --[[ yawVelocity | Line: 40 ]]
	return math.abs((p2.YVector:Dot(p1)))
end
local v1 = 1 / 60
local v2 = setmetatable({}, VRBaseCamera)
v2.__index = v2
function v2.new() --[[ new | Line: 48 | Upvalues: VRBaseCamera (copy), v2 (copy), RunService (copy), v1 (ref) ]]
	local v3 = setmetatable(VRBaseCamera.new(), v2)
	v3:Reset()
	RunService.Stepped:Connect(function(p1, p2) --[[ Line: 53 | Upvalues: v1 (ref) ]]
		v1 = p2
	end)
	return v3
end
function v2.Reset(p1) --[[ Reset | Line: 60 | Upvalues: VehicleCameraCore (copy), Spring (copy), VehicleCameraConfig (copy), CameraUtils (copy) ]]
...
function v2._StepInitialZoom(p1) --[[ _StepInitialZoom | Line: 85 | Upvalues: ZoomController (copy), VehicleCameraConfig (copy) ]]
...
function v2._GetThirdPersonLocalOffset(p1) --[[ _GetThirdPersonLocalOffset | Line: 92 | Upvalues: VehicleCameraConfig (copy) ]]
	return p1.assemblyOffset + Vector3.new(0, p1.assemblyRadius * VehicleCameraConfig.verticalCenterOffset, 0)
end
function v2._GetFirstPersonLocalOffset(p1, p2) --[[ _GetFirstPersonLocalOffset | Line: 96 | Upvalues: LocalPlayer (copy) ]]
...
function v2.Update(p1) --[[ Update | Line: 110 | Upvalues: v1 (ref), mapClamp (copy), LocalPlayer (copy) ]]
	local CurrentCamera = workspace.CurrentCamera
...
function v2.EnterFirstPerson(p1) --[[ EnterFirstPerson | Line: 203 ]]
	p1.inFirstPerson = true
	p1:UpdateMouseBehavior()
end
function v2.LeaveFirstPerson(p1) --[[ LeaveFirstPerson | Line: 208 ]]
