...
local v3, v4 = pcall(function() --[[ Line: 35 ]]
...
local v6 = setmetatable({}, BaseCharacterController)
v6.__index = v6
function v6.new() --[[ new | Line: 52 | Upvalues: BaseCharacterController (copy), v6 (copy) ]]
	local v3 = setmetatable(BaseCharacterController.new(), v6)
...
function v6.GetIsJumping(p1) --[[ GetIsJumping | Line: 87 ]]
	local isJumping = p1.isJumping
	p1.isJumping = false
	return isJumping
end
function v6.Enable(p1, p2, p3) --[[ Enable | Line: 93 | Upvalues: v5 (ref), ContextActionService (copy) ]]
	if p2 == nil then
		return false
	end
...
function v6.OnInputEnded(p1) --[[ OnInputEnded | Line: 122 ]]
	p1.moveTouchObject = nil
	p1.moveVector = Vector3.new(0, 0, 0)
	p1:FadeThumbstick(false)
end
function v6.FadeThumbstick(p1, p2) --[[ FadeThumbstick | Line: 128 | Upvalues: TweenService (copy), v2 (copy), t (copy) ]]
...
function v6.FadeThumbstickFrame(p1, p2, p3) --[[ FadeThumbstickFrame | Line: 171 ]]
	p1.fadeInAndOutHalfDuration = p2 * 0.5
	p1.fadeInAndOutBalance = p3
	p1.tweenInAlphaStart = tick()
end
function v6.InputInFrame(p1, p2) --[[ InputInFrame | Line: 177 ]]
...
function v6.DoFadeInBackground(p1) --[[ DoFadeInBackground | Line: 189 | Upvalues: LocalPlayer (ref) ]]
...
function v6.DoMove(p1, p2) --[[ DoMove | Line: 212 ]]
	local v1
...
function v6.LayoutMiddleImages(p1, p2, p3) --[[ LayoutMiddleImages | Line: 230 | Upvalues: v1 (copy) ]]
...
function v6.MoveStick(p1, p2) --[[ MoveStick | Line: 261 ]]
...
function v6.BindContextActions(p1) --[[ BindContextActions | Line: 269 | Upvalues: TweenService (copy), v5 (ref), ContextActionService (copy), High (copy), UserInputService (copy) ]]
	local function inputBegan(p12) --[[ inputBegan | Line: 270 | Upvalues: p1 (copy), TweenService (ref) ]]
	...
	local function inputChanged(p12) --[[ inputChanged | Line: 302 | Upvalues: p1 (copy) ]]
	...
	local function inputEnded(p12) --[[ inputEnded | Line: 335 | Upvalues: p1 (copy) ]]
		if p12 ~= p1.moveTouchObject then
			return Enum.ContextActionResult.Pass
		end
		p1:OnInputEnded()
	...
	ContextActionService:BindActionAtPriority("DynamicThumbstickAction", function(p12, p2, p3) --[[ handleInput | Line: 345 | Upvalues: inputBegan (copy), v5 (ref), inputChanged (copy), p1 (copy) ]]
	...
	p1.TouchMovedCon = UserInputService.TouchMoved:Connect(function(p1, p2) --[[ Line: 369 | Upvalues: inputChanged (copy) ]]
		inputChanged(p1)
	end)
end
function v6.UnbindContextActions(p1) --[[ UnbindContextActions | Line: 375 | Upvalues: ContextActionService (copy) ]]
	ContextActionService:UnbindAction("DynamicThumbstickAction")
...
function v6.Create(p1, p2) --[[ Create | Line: 383 | Upvalues: v1 (copy), t (copy), RunService (copy), UserInputService (copy), GuiService (copy), LocalPlayer (ref) ]]
	...
	local function layoutThumbstickFrame(p12) --[[ layoutThumbstickFrame | Line: 411 | Upvalues: p1 (copy) ]]
		if p12 then
	...
	local v2 = nil
	local function onCurrentCameraChanged() --[[ onCurrentCameraChanged | Line: 472 | Upvalues: v2 (ref), layoutThumbstickFrame (copy) ]]
		if v2 then
			v2:Disconnect()
			v2 = nil
		end
		...
		local function onViewportSizeChanged() --[[ onViewportSizeChanged | Line: 479 | Upvalues: CurrentCamera (copy), layoutThumbstickFrame (ref) ]]
	...
	p1.onRenderSteppedConn = RunService.RenderStepped:Connect(function() --[[ Line: 499 | Upvalues: p1 (copy) ]]
	...
	p1.onTouchEndedConn = UserInputService.TouchEnded:connect(function(p12) --[[ Line: 518 | Upvalues: p1 (copy) ]]
		if p12 ~= p1.moveTouchObject then
			return
		end
		p1:OnInputEnded()
	end)
	GuiService.MenuOpened:connect(function() --[[ Line: 524 | Upvalues: p1 (copy) ]]
	...
	local function longShowBackground() --[[ longShowBackground | Line: 540 | Upvalues: p1 (copy) ]]
		p1.fadeInAndOutHalfDuration = 2.5
		p1.fadeInAndOutBalance = 0.05
		p1.tweenInAlphaStart = tick()
	end
	local _ = PlayerGui:GetPropertyChangedSignal("CurrentScreenOrientation"):Connect(function() --[[ Line: 546 | Upvalues: v4 (copy), PlayerGui (ref), v3 (ref), p1 (copy) ]]
	...
	if game:IsLoaded() then
		...
		coroutine.wrap(function() --[[ Line: 566 | Upvalues: p1 (copy) ]]
			game.Loaded:Wait()
			p1.fadeInAndOutHalfDuration = 2.5
			p1.fadeInAndOutBalance = 0.05
			p1.tweenInAlphaStart = tick()
