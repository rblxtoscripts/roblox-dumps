...
function update() --[[ update | Line: 5 | Upvalues: activeskills (copy), stringListContains (copy) ]]
	...
	local v2 = 0.3
	for k, v in pairs({ 0.45, 0.55, 0.6 }) do
	...
	local multiJumpCount = script.Parent:WaitForChild("MultiJump"):WaitForChild("multiJumpCount")
	multiJumpCount.Value = if stringListContains(v1, script.Parent.Name .. "/double") then 1 else 0
end
