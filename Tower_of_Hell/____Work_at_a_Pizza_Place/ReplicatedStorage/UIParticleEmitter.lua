...
local v2 = tick()
local v3 = nil
function t.EmittersEnabled(p1, p2) --[[ EmittersEnabled | Line: 14 | Upvalues: v3 (ref), v2 (copy), t2 (copy), t3 (ref), t4 (copy) ]]
	if p2 then
		game:GetService("RunService"):UnbindFromRenderStep("UI_ParticleModule")
		game:GetService("RunService"):BindToRenderStep("UI_ParticleModule", Enum.RenderPriority.First.Value + 1, function(p12) --[[ Line: 17 | Upvalues: v3 (ref), v2 (ref), t2 (ref), t3 (ref), t4 (ref), p1 (copy) ]]
			v3 = tick() - v2
			...
			for k, v in pairs(t3) do
				...
				local v22 = v1 / v.Lifetime
				if v.Lifetime <= v1 then
					table.insert(tbl, v)
					continue
				end
				local v32, v4, v5 = getKeypointsFromTime(v.Transparency, v22)
				v.UI.ImageTransparency = lerpValue(v32.Value, v4.Value, v5)
				local v6, v7, v8 = getKeypointsFromTime(v.Size, v22)
				local v9 = lerpValue(v6.Value, v7.Value, v8) * 100 * v.Scale
				v.UI.Size = UDim2.new(0, v9, 0, v9)
				local v10, v11, v12 = getKeypointsFromTime(v.Emitter.Color, v22)
				v.UI.ImageColor3 = v10.Value:Lerp(v11.Value, v12)
				v.UI.Rotation = v.UI.Rotation + v.RotSpeed * p12
				local Position = v.UI.Position
				local v15 = 1 - math.min(1, v1 ^ 4 * (v.Emitter.Drag / 10))
				local SpreadAngle = v.SpreadAngle
				v.SpeedDisplacement = v.SpeedDisplacement + v.Speed * p12 * v15
				...
				v.Accel = v.Accel + v.Emitter.Acceleration * 100 * v.Scale * v1 ^ 2 * p12 * v15
			...
			t3 = {}
			p1:EmittersEnabled(false)
		end)
	else
		game:GetService("RunService"):UnbindFromRenderStep("UI_ParticleModule")
	end
end
function lerpValue(p1, p2, p3) --[[ lerpValue | Line: 121 ]]
	return p1 + (p2 - p1) * p3
end
function r_num(p1, p2) --[[ r_num | Line: 125 | Upvalues: v1 (copy) ]]
	return v1:NextNumber(if p2 and p1 then p1 else 0, p2 or p1)
end
function r_int(p1, p2) --[[ r_int | Line: 129 | Upvalues: v1 (copy) ]]
	return v1:NextInteger(if p2 and p1 then p1 else 0, p2 or p1)
end
function addParticle(p1, p2, p3) --[[ addParticle | Line: 133 | Upvalues: t4 (copy), t3 (ref) ]]
	local v1 = nil
	for k, v in pairs(t4) do
		if v.UI then
			v1 = v.UI
...
function getRandomNumberSequence(p1) --[[ getRandomNumberSequence | Line: 188 ]]
	local t = {
		Keypoints = {}
	}
...
function getKeypointsFromTime(p1, p2) --[[ getKeypointsFromTime | Line: 198 ]]
	for k, v in pairs(p1.Keypoints) do
		if p2 < v.Time then
...
function t.AddEmitter(p1, p2, p3) --[[ AddEmitter | Line: 213 | Upvalues: t2 (copy) ]]
	table.insert(t2, {
		Emitter = p2,
		lastEmission = tick(),
		Scale = p3 or 1
	})
	p1:EmittersEnabled(true)
end
function t.RemoveEmitter(p1, p2) --[[ RemoveEmitter | Line: 219 | Upvalues: t2 (copy) ]]
	local tbl = {}
	for k, v in pairs(t2) do
