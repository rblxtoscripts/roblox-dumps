...
local t = {}
local t2 = {}
local function getSoundsFolder() --[[ getSoundsFolder | Line: 29 | Upvalues: ReplicatedStorage (copy), v1 (copy) ]]
...
function t.AddButton(p1, p2, p3, p4, p5, p6) --[[ AddButton | Line: 41 | Upvalues: TweenModuleBlur (copy), t2 (copy), TweenService (copy), v1 (copy), getSoundsFolder (copy), SoundService (copy), ReplicatedStorage (copy) ]]
	local Size = p2.Size
	if p2 == "Blur" then
		...
		if typeof(p2) ~= "string" then
			p2.MouseButton1Down:Connect(function() --[[ Line: 96 | Upvalues: v4 (ref), TweenService (ref), p2 (copy), Size (copy) ]]
				if v4 then
			...
			p2.MouseLeave:Connect(function() --[[ Line: 106 | Upvalues: p5 (copy), TweenService (ref), p2 (copy), v1 (ref), Size (copy), v4 (ref) ]]
				if p5 then
			...
			p2.MouseEnter:Connect(function() --[[ Line: 118 | Upvalues: p5 (copy), TweenService (ref), p2 (copy), v1 (ref), Size (copy), v4 (ref) ]]
				if p5 then
			...
			p2.MouseButton1Up:Connect(function() --[[ Line: 130 | Upvalues: v4 (ref), TweenService (ref), v1 (ref), p2 (copy), Size (copy) ]]
				if v4 then
					TweenService:Create(v4, TweenInfo.new(v1.Time, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {
						Scale = 1
					}):Play()
			...
			p2.MouseButton1Click:Connect(function() --[[ Line: 138 | Upvalues: v1 (ref), getSoundsFolder (ref), p4 (copy), SoundService (ref), p3 (copy), ReplicatedStorage (ref), t2 (ref), TweenService (ref), TweenModuleBlur (ref) ]]
				...
				if p3.Visible then
					...
					for v12, v2 in t2 do
						if v2.Visible then
							if pcall(function() --[[ Line: 157 | Upvalues: TweenService (ref), v2 (copy), v1 (ref) ]]
								TweenService:Create(v2, TweenInfo.new(v1.Time, Enum.EasingStyle[v1.FrameStyle], Enum.EasingDirection.Out), {
									Position = UDim2.new(0.5, 0, -2, 0)
								}):Play()
							end) then
								task.delay(0.3, function() --[[ Line: 164 | Upvalues: v2 (copy) ]]
									v2.Visible = false
								end)
								continue
							end
		...
		elseif p2 == "Open" then
			for v5, v6 in t2 do
				if v6.Visible then
					if pcall(function() --[[ Line: 69 | Upvalues: TweenService (ref), v6 (copy), v1 (ref) ]]
						TweenService:Create(v6, TweenInfo.new(v1.Time, Enum.EasingStyle[v1.FrameStyle], Enum.EasingDirection.Out), {
							Position = UDim2.new(0.5, 0, -2, 0)
						}):Play()
					end) then
						task.delay(0.3, function() --[[ Line: 76 | Upvalues: v6 (copy) ]]
							v6.Visible = false
						end)
						continue
					end
