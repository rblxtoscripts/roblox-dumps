...
local v3 = 0
function animation(p1, p2, p3) --[[ animation | Line: 26 | Upvalues: MainFrame (copy), v2 (ref), v3 (ref), ReplicatedStorage (copy), TweenService (copy), Size (copy), Enchants (copy), Size_2 (copy) ]]
	if p3 then
		MainFrame.RerollsFrame.TextLabel.Text = p3
	end
	if p1 == "StopAuto" then
		v2 = false
		return
...
function update() --[[ update | Line: 62 | Upvalues: ClientState (copy), MainFrame (copy) ]]
	local v1 = ClientState.GetData()
	MainFrame.RerollsFrame.TextLabel.Text = if v1.Items.Souls then v1.Items.Souls else 0
end
MainFrame.RollEnchant.MouseButton1Click:Connect(function() --[[ Line: 67 | Upvalues: v2 (ref), ClientState (copy), MainFrame (copy), Enchants (copy), Enchant (copy) ]]
	if v2 then
		return
	end
...
MainFrame.AutoEnchant.MouseButton1Click:Connect(function() --[[ Line: 83 | Upvalues: v2 (ref), ClientState (copy), MainFrame (copy), Enchants (copy), Enchant (copy) ]]
	v2 = not v2
	if not v2 then
		return
	end
...
function updateTowerSlot(p1) --[[ updateTowerSlot | Line: 106 | Upvalues: Tween (copy), MainFrame (copy), ClientState (copy), Towers (copy), ReplicatedStorage (copy), Enchants (copy), Misc (copy) ]]
	if p1 == 1 then
		return
	end
...
MainFrame.TowerSlot.MouseButton1Click:Connect(function() --[[ Line: 144 | Upvalues: Enchant (copy), LocalPlayer (copy) ]]
...
function initChances() --[[ initChances | Line: 153 | Upvalues: MainFrame (copy), Tween (copy), ClientState (copy), v1 (ref) ]]
	for k, v in pairs(MainFrame.ChancesScroll:GetChildren()) do
		if v:IsA("ImageButton") then
			Tween:AddButton(v)
			v.MouseButton1Click:Connect(function() --[[ Line: 158 | Upvalues: ClientState (ref), v1 (ref), v (copy), MainFrame (ref) ]]
...
MainFrame.SelectedChance.Changed:Connect(function() --[[ Line: 173 | Upvalues: Enchants (copy), MainFrame (copy), t (copy) ]]
...
MainFrame.ChancesFrame.Exit.MouseButton1Click:Connect(function() --[[ Line: 197 | Upvalues: MainFrame (copy) ]]
	MainFrame.SelectedChance.Value = ""
end)
MainFrame.ConfirmFrame.No.MouseButton1Click:Connect(function() --[[ Line: 201 | Upvalues: MainFrame (copy) ]]
	MainFrame.ConfirmFrame.Visible = false
end)
local v4 = false
MainFrame.ConfirmFrame:GetPropertyChangedSignal("Visible"):Connect(function() --[[ Line: 206 | Upvalues: v4 (ref) ]]
	v4 = true
	task.wait(0.8)
	v4 = false
end)
MainFrame:GetPropertyChangedSignal("Visible"):Connect(function() --[[ Line: 212 | Upvalues: MainFrame (copy), Enchant (copy) ]]
	if MainFrame.Visible then
		return
	end
	Enchant:FireServer(MainFrame.TowerID.Value, "Left")
end)
MainFrame.ConfirmFrame.Yes.MouseButton1Click:Connect(function() --[[ Line: 218 | Upvalues: v4 (ref), MainFrame (copy), Enchant (copy) ]]
	if v4 then
		return
	end
	MainFrame.ConfirmFrame.Visible = false
...
ReplicatedStorage.Remotes.Misc.EnchantTouch.OnClientEvent:Connect(function() --[[ Line: 225 | Upvalues: MainFrame (copy), Tween (copy) ]]
...
ReplicatedStorage.Remotes.Towers.MetaData.OnClientEvent:Connect(function(p1) --[[ Line: 231 | Upvalues: Towers (copy) ]]
	if not p1 then
		return
	end
	if pcall(function() --[[ Line: 233 | Upvalues: p1 (ref), Towers (ref) ]]
...
function initBuy() --[[ initBuy | Line: 246 | Upvalues: MainFrame (copy), Robux (copy), Tween (copy), PolicyService (copy), LocalPlayer (copy), MarketplaceService (copy) ]]
	for k, v in pairs(MainFrame.BuyTokens:GetChildren()) do
		if Robux.Products[v.Name] then
			Tween:AddButton(v, nil, nil, nil, true)
			v.MouseButton1Click:Connect(function() --[[ Line: 250 | Upvalues: PolicyService (ref), LocalPlayer (ref), MarketplaceService (ref), Robux (ref), v (copy) ]]
				local v1, v2 = pcall(function() --[[ Line: 252 | Upvalues: PolicyService (ref), LocalPlayer (ref) ]]
			...
			pcall(function() --[[ Line: 265 | Upvalues: v (copy), MarketplaceService (ref), Robux (ref) ]]
				v.Robux.Text = ("R$ %*"):format(MarketplaceService:GetProductInfo(Robux.Products[v.Name], Enum.InfoType.Product).PriceInRobux)
			end)
		end
	end
end
function updateAutoSkip() --[[ updateAutoSkip | Line: 273 | Upvalues: ClientState (copy), MainFrame (copy), t (copy) ]]
	local v1 = ClientState.GetData()
	for k, v in pairs(MainFrame.ChancesScroll:GetChildren()) do
		local v2
		if v:IsA("ImageButton") then
...
MainFrame.AutoSkip.MouseButton1Click:Connect(function() --[[ Line: 286 | Upvalues: v1 (ref), MainFrame (copy) ]]
	v1 = not v1
	if v1 then
