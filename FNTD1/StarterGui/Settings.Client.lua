...
function update() --[[ update | Line: 29 | Upvalues: ClientState (copy), Settings (copy), t (copy), SpecialSettings (copy) ]]
	...
	for k, v in pairs(Settings.ScrollingFrame:GetChildren()) do
		if v:IsA("Frame") and (v.Name ~= "Codes" and not v:FindFirstChild("NotSetting")) then
...
function init() --[[ init | Line: 56 | Upvalues: Settings (copy), Tween (copy), ReplicatedStorage (copy), ClientState (copy), UserInputService (copy) ]]
	for k, v in pairs(Settings.ScrollingFrame:GetChildren()) do
		if v:IsA("Frame") and not v:FindFirstChild("NotSetting") then
			Tween:AddButton(v.EquipBest)
			v.EquipBest.MouseButton1Click:Connect(function() --[[ Line: 61 | Upvalues: v (copy), ReplicatedStorage (ref) ]]
...
ReplicatedStorage.Remotes.Misc.Codes.OnClientEvent:Connect(function(p1) --[[ Line: 77 | Upvalues: Settings (copy) ]]
	if p1 == true then
...
init()
update()
Tween:AddButton(Settings.Exit, Settings)
Tween:AddButton(script.Parent.Parent.SideDock.Container.SettingsButton, Settings)
