...
function returnSumOfWeight(p1) --[[ returnSumOfWeight | Line: 15 ]]
	local sum = 0
...
function getRandomItem(p1) --[[ getRandomItem | Line: 23 ]]
...
function t.GetLetter() --[[ GetLetter | Line: 35 | Upvalues: Config (copy) ]]
	return getRandomItem(Config)
end
local t2 = {}
function t.Roll(p1, p2, p3, p4) --[[ Roll | Line: 40 | Upvalues: v1 (copy), t2 (copy), ReplicatedStorage (copy), v2 (copy), Config (copy), t (copy) ]]
	...
	t2[p1.Name] = true
	task.delay(0.5, function() --[[ Line: 48 | Upvalues: t2 (ref), p1 (copy) ]]
		t2[p1.Name] = nil
	end)
	...
	if v22.Stats then
		...
		for k, v in pairs(Stats) do
			local Chance = Config[v].Chance
			if Chance <= 1 and not (p3[k] or p4) then
				print(Chance, p3[k], p4)
				return
			end
		end
		...
		local t3 = {}
		t3.Damage = if p3 and (p3.Damage and Stats.Damage) then Stats.Damage else t.GetLetter()
		t3.Range = if p3 and (p3.Range and Stats.Range) then Stats.Range else t.GetLetter()
		t3.Cooldown = if p3 and (p3.Cooldown and Stats.Cooldown) then Stats.Cooldown else t.GetLetter()
		v12.Data.Towers[p2].Stats = t3
...
function t.GetLetterStats(p1, p2, p3) --[[ GetLetterStats | Line: 122 | Upvalues: Config (copy) ]]
	local t = {
		Letter = p2,
...
function t.GetStatMulti(p1, p2, p3, p4) --[[ GetStatMulti | Line: 139 | Upvalues: LocalPlayer (copy), v1 (copy), Config (copy) ]]
...
if not LocalPlayer and ReplicatedStorage.Remotes.Misc:FindFirstChild("StatReroll") then
	ReplicatedStorage.Remotes.Misc.StatReroll.OnServerEvent:Connect(function(p1, p2, p3, p4) --[[ Line: 181 | Upvalues: t (copy) ]]
