...
local v1 = LocalPlayer:GetRankInGroup(33298472)
if v1 ~= 254 and (v1 ~= 55 and (v1 ~= 15 and v1 ~= 255)) then
	script:Destroy()
end
local abs = math.abs
...
Workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(function() --[[ Line: 29 | Upvalues: Workspace (copy), CurrentCamera (ref) ]]
...
local t2 = {}
t2.__index = t2
function t2.new(p1, p2) --[[ new | Line: 57 | Upvalues: t2 (copy) ]]
...
function t2.Update(p1, p2, p3) --[[ Update | Line: 65 | Upvalues: exp (copy) ]]
...
function t2.Reset(p1, p2) --[[ Reset | Line: 82 ]]
	p1.p = p2
	p1.v = p2 * 0
end
...
local v8 = t2.new(4, 0)
local t3 = {}
local function fCurve(p1) --[[ fCurve | Line: 105 | Upvalues: exp (copy) ]]
	return (exp(2 * p1) - 1) / (math.exp(2) - 1)
end
local function fDeadzone(p1) --[[ fDeadzone | Line: 109 | Upvalues: exp (copy) ]]
	return (exp(2 * ((p1 - 0.15) / 0.85)) - 1) / (math.exp(2) - 1)
end
local function thumbstickCurve(p1) --[[ thumbstickCurve | Line: 113 | Upvalues: sign (copy), abs (copy), exp (copy), clamp (copy) ]]
...
local v9 = Vector2.new(1, 1) * 0.04908738521234052
local v10 = Vector2.new(1, 1) * 0.39269908169872414
local v11 = 1
function t3.Vel(p1) --[[ Vel | Line: 164 | Upvalues: v11 (ref), t5 (copy), clamp (copy), thumbstickCurve (ref), t4 (copy), UserInputService (copy) ]]
	v11 = clamp(v11 + p1 * (t5.Up - t5.Down) * 0.75, 0.01, 4)
	...
	return (v5 + v9) * (v11 * (if UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) or UserInputService:IsKeyDown(Enum.KeyCode.RightShift) then 0.25 else 1))
end
function t3.Pan(p1) --[[ Pan | Line: 184 | Upvalues: thumbstickCurve (ref), t4 (copy), v10 (copy), t6 (copy), v9 (copy) ]]
	local v1 = Vector2.new(thumbstickCurve(t4.Thumbstick2.Y), thumbstickCurve(-t4.Thumbstick2.X)) * v10
	local v2 = t6.Delta * v9
	t6.Delta = Vector2.new()
	return v1 + v2
end
function t3.Fov(p1) --[[ Fov | Line: 194 | Upvalues: t4 (copy), t6 (copy) ]]
	local v2 = t6.MouseWheel * 1
	t6.MouseWheel = 0
	return (t4.ButtonX - t4.ButtonY) * 0.25 + v2
end
local function Keypress(p1, p2, p3) --[[ Keypress | Line: 202 | Upvalues: t5 (copy) ]]
	t5[p3.KeyCode.Name] = if p2 == Enum.UserInputState.Begin then 1 else 0
	return Enum.ContextActionResult.Sink
end
local function GpButton(p1, p2, p3) --[[ GpButton | Line: 207 | Upvalues: t4 (copy) ]]
	t4[p3.KeyCode.Name] = if p2 == Enum.UserInputState.Begin then 1 else 0
	return Enum.ContextActionResult.Sink
end
local function MousePan(p1, p2, p3) --[[ MousePan | Line: 212 | Upvalues: t6 (copy) ]]
	local Delta = p3.Delta
	t6.Delta = Vector2.new(-Delta.y, -Delta.x)
	return Enum.ContextActionResult.Sink
end
local function Thumb(p1, p2, p3) --[[ Thumb | Line: 218 | Upvalues: t4 (copy) ]]
	t4[p3.KeyCode.Name] = p3.Position
	return Enum.ContextActionResult.Sink
end
local function Trigger(p1, p2, p3) --[[ Trigger | Line: 223 | Upvalues: t4 (copy) ]]
	t4[p3.KeyCode.Name] = p3.Position.z
	return Enum.ContextActionResult.Sink
end
local function MouseWheel(p1, p2, p3) --[[ MouseWheel | Line: 228 | Upvalues: t6 (copy) ]]
	t6[p3.UserInputType.Name] = -p3.Position.z
	return Enum.ContextActionResult.Sink
end
local function Zero(p1) --[[ Zero | Line: 233 ]]
	for k, v in pairs(p1) do
		p1[k] = v * 0
	end
end
function t3.StartCapture() --[[ StartCapture | Line: 239 | Upvalues: ContextActionService (copy), Keypress (copy), High (copy), MousePan (copy), MouseWheel (copy), GpButton (copy), Trigger (copy), Thumb (copy) ]]
...
function t3.StopCapture() --[[ StopCapture | Line: 256 | Upvalues: v11 (ref), t4 (copy), t5 (copy), t6 (copy), ContextActionService (copy) ]]
	v11 = 1
	local v1 = t4
...
local function StepFreecam(p1) --[[ StepFreecam | Line: 303 | Upvalues: v6 (copy), t3 (copy), v7 (copy), v8 (copy), v5 (ref), rad (copy), tan (copy), sqrt (copy), clamp (copy), v4 (ref), v2 (copy), v3 (ref), CurrentCamera (ref), GetFocusDistance (copy) ]]
	...
	local v52 = CFrame.new(v3) * CFrame.fromOrientation(v4.x, v4.y, 0) * CFrame.new(v1 * Vector3.new(64, 64, 64) * p1)
	v3 = v52.p
...
function t7.Push() --[[ Push | Line: 344 | Upvalues: t9 (copy), StarterGui (copy), t10 (copy), LocalPlayer_2 (ref), t8 (copy), v17 (ref), CurrentCamera (ref), v14 (ref), v16 (ref), v15 (ref), v13 (ref), UserInputService (copy), v12 (ref) ]]
	...
	local PlayerGui = LocalPlayer_2:FindFirstChildOfClass("PlayerGui")
	if not PlayerGui then
		v17 = CurrentCamera.FieldOfView
		CurrentCamera.FieldOfView = 70
		v14 = CurrentCamera.CameraType
		CurrentCamera.CameraType = Enum.CameraType.Custom
		v16 = CurrentCamera.CFrame
		v15 = CurrentCamera.Focus
		v13 = UserInputService.MouseIconEnabled
		UserInputService.MouseIconEnabled = false
		v12 = UserInputService.MouseBehavior
...
function t7.Pop() --[[ Pop | Line: 380 | Upvalues: t9 (copy), StarterGui (copy), t10 (copy), t8 (copy), CurrentCamera (ref), v17 (ref), v14 (ref), v16 (ref), v15 (ref), UserInputService (copy), v13 (ref), v12 (ref) ]]
	...
	CurrentCamera.FieldOfView = v17
	v17 = nil
	CurrentCamera.CameraType = v14
	v14 = nil
	CurrentCamera.CFrame = v16
	v16 = nil
	CurrentCamera.Focus = v15
	v15 = nil
	UserInputService.MouseIconEnabled = v13
	v13 = nil
	UserInputService.MouseBehavior = v12
	v12 = nil
end
local function StartFreecam() --[[ StartFreecam | Line: 413 | Upvalues: CurrentCamera (ref), v4 (ref), v3 (ref), v5 (ref), v6 (copy), v7 (copy), v8 (copy), t7 (copy), RunService (copy), StepFreecam (copy), t3 (copy) ]]
	local v1 = CurrentCamera.CFrame
	v4 = Vector2.new(v1:toEulerAnglesYXZ())
	v3 = v1.p
...
local function StopFreecam() --[[ StopFreecam | Line: 428 | Upvalues: t3 (copy), RunService (copy), t7 (copy) ]]
	t3.StopCapture()
	RunService:UnbindFromRenderStep("Freecam")
	t7.Pop()
end
local v18 = false
local function ToggleFreecam() --[[ ToggleFreecam | Line: 439 | Upvalues: v18 (ref), t3 (copy), RunService (copy), t7 (copy), StartFreecam (copy) ]]
	if v18 then
		t3.StopCapture()
		RunService:UnbindFromRenderStep("Freecam")
		t7.Pop()
...
local function CheckMacro(p1) --[[ CheckMacro | Line: 448 | Upvalues: UserInputService (copy), v18 (ref), t3 (copy), RunService (copy), t7 (copy), StartFreecam (copy) ]]
	for i = 1, #p1 - 1 do
...
ContextActionService:BindActionAtPriority("FreecamToggle", function(p1, p2, p3) --[[ HandleActivationInput | Line: 457 | Upvalues: t (copy), LocalPlayer (copy), CheckMacro (copy) ]]
